...
- Vídeos não-autoplay passam `src/srcset` para `data-*` via shortcode e podem ser carregados por integração de interação; mídia crítica é preservada. Imagens recebem lazy/async. Arquivos >300 KB devem ser tratados manualmente com variantes WebP/AVIF e dimensões.
