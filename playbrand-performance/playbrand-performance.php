...
final class Plugin { public static function boot(): void { if (\is_admin() && !\wp_doing_ajax()) Admin::init(); if (defined('PBPF_SAFE_MODE') && PBPF_SAFE_MODE) return; if (!self::safe_context()) return; Assets::init(); Fonts::init(); Media::init(); ThirdParty::init(); ResourceHints::init(); \add_action('init',[self::class,'cleanup']); }
