...
- Seletor `[pgw_categorias]` ganhou layout compacto premium, Maven Pro 14px, paleta neon distinta e campo administrativo de cor por categoria.
- “Entrar no Grupo” agora exige autenticação em cards, página individual e rota direta, preservando o convite como destino após o OTP.
- Categorias deixaram de exibir “Ver mais”, adotaram tipografia preta Maven Pro 14px/800, e a página individual passou a remover a miniatura duplicada e centralizar o título em 40px.
- Login e cadastro passaram a usar endpoint dedicado `admin-post`, corrigindo recarregamentos sem criação da conta ou avanço ao OTP.
- Cards ganharam título clicável para a descrição, e o novo `[minha-conta-header]` apresenta foto, nome e dropdown completo de conta no cabeçalho.
- Auditoria do cadastro elevou o schema para v5, autorrecupera tabelas/papéis ausentes, valida a persistência do desafio OTP e fornece fallback da confirmação pela página Entrar.
- Modal renova provas HMAC por AJAX para continuar funcional mesmo quando seu HTML foi servido por cache do Elementor ou de página.
- Corrigido fluxo pós-OTP para preservar `pgw_redirect_to` na URL de confirmação e usar esse destino como fallback após validar o código, garantindo retorno ao grupo solicitado.
- Refinado formulário de autenticação em 2 fatores com layout compacto, responsivo e sem sobreposição de botões.
...
- Confirmação de OTP agora é processada em `template_redirect` antes de qualquer saída do tema/Elementor, evitando avisos “Cannot modify header information” ao definir cookie e redirecionar após o código.
