...
### Candidato source-level reproduzível (2.7.0)

`tools/build_aiko_home_css.py` extrai o `aiko.zip` intacto, remove somente os imports Sass de WooCommerce do framework/tema e `single-product`, e compila `assets/aiko-home.css` com Dart Sass comprimido. A compilação completa equivalente mede 805.211 B; o candidato mede 613.013 B, uma redução de 192.198 B (23,9%) antes de compressão. O SHA-256 do CSS é `591f30eecd35619786ffbf4dce3a24370df50b0a619f704213e7ab21a3115941`.

O builder fixa Dart Sass 1.102.0 e aceita somente o archive auditado com SHA-256 `7619fac210fdaf70720cd2ce75b9a866f2f7db719e2b87aaf2e671f3c524752e`. Com gzip `-9 -n`, a compilação completa mede 82.122 B e o candidato 63.751 B (−18.371 B); esses números são de build local reproduzível, não bytes de rede pós-W3TC.

O asset é opt-in, exclusivo da Home Aiko sem child theme e conserva o handle `aiko-style`; portanto o CSS inline do tema continua associado ao mesmo handle. A URL inclui `?ver=JPP_VERSION` para invalidar navegador/CDN em updates. Mesmo após ativação, contextos WooCommerce, blocos/shortcodes de comércio e widgets WooCommerce em sidebars ativas (inclusive mini-cart) restauram automaticamente o stylesheet original. Widgets inativos/órfãos não causam falso fallback. Markup de comércio produzido por código customizado ainda exige regressão visual em WordPress staging.
