...
final class RMA_Core_Entities {
    ...
    public function __construct() {
        ...
        add_action('wp_footer', [$this, 'inject_public_profile_site_block'], 999);
    }
    ...
    public function inject_public_profile_site_block(): void {
        ...
        if ($request_path === '' || strpos($request_path, '/employer') !== 0) {
            return;
        }

        $entity_id = $this->resolve_entity_id_for_profile_context($request_path);
        if ($entity_id <= 0) {
            return;
        }
        ...
        (function(){
            ...
            var detailsList = document.querySelector('.fr-c-details .fr-ca-more-details ul, .fr-c-details .fr-c-more-details ul, .fr-ca-details .fr-ca-more-details ul, .fr-ca-more-details ul');
            if (!detailsList || detailsList.tagName !== 'UL') return;

            var viewsNode = Array.prototype.slice.call(detailsList.querySelectorAll('li .fr-c-full-details span, li .fr-ca-full-details span')).find(function (label) {
                return /visualiza[çc][õo]es/i.test((label.textContent || '').trim());
            });

            var blockClass = viewsNode && viewsNode.closest('.fr-ca-full-details') ? 'fr-ca-full-details' : 'fr-c-full-details';
            var li = document.createElement('li');
            li.setAttribute('data-rma-site-url', '1');
            li.innerHTML = '<div class="' + blockClass + '"><span>Site</span><p><a href="' + siteUrl.replace(/"/g, '&quot;') + '" target="_blank" rel="noopener noreferrer nofollow">' + siteUrl + '</a></p></div>';

            var beforeLi = viewsNode ? viewsNode.closest('li') : null;
            if (beforeLi && beforeLi.parentElement === detailsList) {
                detailsList.insertBefore(li, beforeLi);
            } else {
    ...
    public function inject_dashboard_site_sync_script(): void {
        ...
        $endpoint = esc_url_raw(rest_url('rma/v1/entities/' . $entity_id . '/site-url'));
        ?>
        <script>
        (function(){
