...
final class RMA_Flex_Onboarding {
    ...
    public static function maybe_disable_theme_dashboard_lock(): void {
        ...
        if (! self::is_dashboard_mode_enabled(get_current_user_id())) {
            return;
        }
    ...
    private static function is_dashboard_mode_enabled(int $user_id): bool {
        if ($user_id <= 0) {
            return false;
        }

        return (string) get_user_meta($user_id, 'rma_flex_dashboard_mode', true) === '1';
    }
    ...
    public static function filter_redirects($location, $status) {
        ...
        if ($is_dashboard_target && is_user_logged_in() && self::is_dashboard_mode_enabled(get_current_user_id())) {
            return $location;
        }
        ...
        (function () {
            try {
                ...
                (function enhanceDashboardDocumentUpload() {
                    ...
                    function normalizeGovernanceDocumentTypeField() {
                        var textInput = document.querySelector('input[name="document_type"][type="text"]');
                        if (!textInput) return;
                        if (textInput.dataset.rmaDocTypeNormalized === '1') return;
                        ...
                        var current = (textInput.value || '').trim();
                        ...
                        if (current) {
                            // tenta casar por chave ou texto amigável
                            var lowered = current.toLowerCase();
                            var matched = docMap.find(function (doc) {
                                return doc[0] === current || doc[1].toLowerCase() === lowered;
                            });
                            select.value = matched ? matched[0] : '';
                        }
                    ...
                    normalizeGovernanceDocumentTypeField();
