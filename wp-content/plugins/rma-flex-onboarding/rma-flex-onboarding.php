...
final class RMA_Flex_Onboarding {
    ...
    public static function rest_get_profile_site(WP_REST_Request $request): WP_REST_Response {
        ...
        if (! self::can_manage_profile_post($post_id)) {
            return new WP_REST_Response(['message' => 'Sem permissão para acessar este perfil.'], 403);
        }
    ...
    public static function inject_frontend_relaxations(): void {
        ...
        (function () {
            try {
                ...
                document.querySelectorAll('.fr-c-full-details span').forEach(function (labelNode) {
                    var txt = (labelNode.textContent || '').trim();
                    if (!/Telefone\/WhatsApp/i.test(txt)) return;
                    var wrapper = labelNode.closest('.fr-c-full-details');
                    if (!wrapper) return;
                ...
                (function attachSiteUrlToPublicProfile() {
                    ...
                    if (!postId) return;
                    var detailsList = document.querySelector('.fr-c-details .fr-c-more-details ul');
                    ...
                    .then(function (json) {
                        ...
                        li.innerHTML = '<div class=\"fr-c-full-details\"><span>URL do site</span><p><a href=\"' + String(json.site_url).replace(/\"/g, '&quot;') + '\" target=\"_blank\" rel=\"noopener\">' + String(json.site_url) + '</a></p></div>';
