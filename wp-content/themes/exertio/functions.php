...
add_action('wp_enqueue_scripts', function () {
    ...
    if (file_exists($js_file)) {
        ...
        wp_localize_script('rma-glass-theme-js', 'rmaGlassTheme', [
            'checkoutUrl' => (string) apply_filters('rma_checkout_url', home_url('/checkout/')),
        ]);
    }
...
add_action('wp_footer', function (): void {
    ...
    (function(){
        ...
        var contactEmail = <?php echo wp_json_encode($contact_email); ?>;
        ...
        function injectPublicSiteUrl() {
            ...
            if (wrapList && wrapList.tagName === 'UL') {
                ...
                var viewsNode = Array.prototype.slice.call(wrapList.querySelectorAll('li .fr-c-full-details span, li .fr-ca-full-details span')).find(function (label) {
                    return /visualiza[çc][õo]es/i.test((label.textContent || '').trim());
                });
                var beforeLi = viewsNode ? viewsNode.closest('li') : null;
                if (beforeLi && beforeLi.parentElement === wrapList) {
                    wrapList.insertBefore(li, beforeLi);
                } else {
...
add_action('wp_ajax_rma_save_contact_email', function () {
    if (!is_user_logged_in()) {
        wp_send_json_error(array('message' => 'Unauthorized'), 403);
    }
    check_ajax_referer('wp_rest', 'security');
    $entity_id = isset($_POST['entity_id']) ? (int) $_POST['entity_id'] : 0;
    $contact_email_raw = isset($_POST['contact_email']) ? wp_unslash((string) $_POST['contact_email']) : '';
    $contact_email = sanitize_email($contact_email_raw);

    if ($contact_email_raw !== '' && !is_email($contact_email)) {
        wp_send_json_error(array('message' => 'E-mail de contato inválido.'), 400);
    }

    if ($entity_id <= 0 || (int) get_post_field('post_author', $entity_id) !== get_current_user_id()) {
        wp_send_json_error(array('message' => 'Sem permissão para editar esse perfil.'), 403);
    }
