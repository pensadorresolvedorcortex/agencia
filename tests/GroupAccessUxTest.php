...
foreach (['group_join_button', 'whatsapp_icon', 'pgw-button__icon', 'pgw-open-login', 'data-pgw-auth-redirect', 'rel="nofollow noopener noreferrer external"', 'if(!is_user_logged_in()){$target=', 'suppress_theme_group_thumbnail', 'wp_get_attachment_image($thumbnail_id', 'self::group_join_button($id)'] as $rule) if (strpos($php,$rule)===false) throw new RuntimeException('Missing group access contract: '.$rule);
...
foreach (['.single-pgw_group .default-blog__item-single .blog__details_title', 'font-size:40px', 'text-align:center', '.pgw-single__image', '.pgw-button__icon'] as $rule) if (strpos($css,$rule)===false) throw new RuntimeException('Missing single group CSS: '.$rule);
echo "22 group access and single view checks passed.\n";
