...
foreach (["'show_more'=>0", 'pgw_category_color', 'category_color_style'] as $rule) if (strpos($php, $rule) === false) throw new RuntimeException('Missing category UI contract: '.$rule);
foreach (['padding:2px 0 42px', 'font-size:14px', '--pgw-category-color', 'color:#000', 'font-weight:800'] as $rule) if (strpos($css, $rule) === false) throw new RuntimeException('Missing category CSS contract: '.$rule);
