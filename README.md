...
### Confirmação de OTP antes do HTML

A submissão do formulário de confirmação (`pgw_verify_nonce` + `pgw_flow`) é interceptada em `template_redirect`, antes de o tema imprimir estilos ou conteúdo. Assim, `wp_set_auth_cookie()` e `wp_safe_redirect()` rodam com cabeçalhos livres e não geram avisos “Cannot modify header information” após inserir o código.
