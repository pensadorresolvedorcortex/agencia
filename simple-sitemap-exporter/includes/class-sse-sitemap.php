...
class SSE_Sitemap
{
    ...
    public function build_payload($urls)
    {
        ...
        $chunk_size = self::MAX_URLS;

        if ($total_urls > self::MAX_URLS) {
            $chunk_size = (int) ceil($total_urls / self::TARGET_PARTS);
            $chunk_size = max(1, min(self::MAX_URLS, $chunk_size));
        }

        $chunks = array_chunk($prepared, $chunk_size);
    ...
    public function maybe_render_public_sitemap()
    {
        ...
        if ('1' === get_query_var('sse_sitemap')) {
            ...
            if (preg_match('#/sitemap-([0-9]+)\.xml$#', $path, $matches)) {
                $filename = sprintf('sitemap-%d.xml', absint($matches[1]));
            } elseif (preg_match('#/sitemap\.xml$#', $path)) {
