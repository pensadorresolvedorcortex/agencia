Blog Privilège AI Premium Final v4.4.1

Alterações v4.4.1 — correção crítica de compatibilidade:
- Remove a chamada ao serviço interno inexistente `Yoast\\WP\\SEO\\Builders\\Indexable_Builder`, responsável pelo erro fatal ao abrir o painel.
- Mantém a integração exclusivamente pelos campos públicos persistidos pelo Yoast no `postmeta`, sem acessar o container interno do plugin.
- Atualiza a migração para reaplicar a correção com segurança aos artigos gerados anteriormente.

Alterações v4.4.0 — critérios completos de SEO e legibilidade:
- Garante a frase-chave no título do post, título SEO, slug, meta description, introdução, subtítulo e em quatro a dez ocorrências naturais no conteúdo.
- Valida pelo menos 400 palavras antes de publicar e distribui o texto em parágrafos curtos, H2/H3, listas e frases mais diretas.
- Inclui conectivos de transição como “além disso”, “em seguida”, “portanto”, “assim”, “por fim” e “contudo”.
- Adiciona um link interno para o próprio site e um link externo para a documentação de conteúdo útil do Google Search Central.
- Reprova a publicação quando o contrato SEO mínimo não estiver completo, em vez de apenas preencher uma pontuação visual.

Alterações v4.3.2 — histórico:
- A tentativa de reconstrução direta de `Indexable` foi removida na versão 4.4.1 por depender de uma API interna instável do Yoast.
- Amplia a correção automática para os 250 artigos mais recentes gerados pelo plugin.

Alterações v4.3.1 — correção do indicador cinza do Yoast:
- Grava os scores `_yoast_wpseo_linkdex` e `_yoast_wpseo_content_score`, que não são calculados automaticamente porque posts do cron não passam pelo editor JavaScript do Yoast.
- Sincroniza novamente os metadados após finalizar imagem, conteúdo e publicação.
- Executa uma migração única no painel para corrigir até 100 artigos já gerados que ainda aparecem com indicadores cinza.
- Limita cada artigo a oito tags realmente relacionadas, evitando dezenas de arquivos de taxonomia rasos.

Alterações v4.3.0 — otimização para Yoast SEO:
- Frase-chave de foco curta, específica e gerada automaticamente para cada pauta.
- Integração nativa com os campos `_yoast_wpseo_focuskw`, `_yoast_wpseo_title` e `_yoast_wpseo_metadesc`.
- Frase-chave posicionada no início do título SEO, meta description, slug, introdução e excerpt.
- Frase-chave incluída em subtítulo H2 e no texto alternativo da imagem destacada.
- Títulos SEO limitados a 60 caracteres e descrições limitadas a 155 caracteres para evitar cortes no snippet.
- Slugs continuam curtos e agora começam pela frase-chave de foco.
- O conteúdo mantém mais de 2.500 caracteres, múltiplos subtítulos, lista prática e distribuição natural do tema.

Observação: o semáforo do Yoast também considera configurações externas ao plugin, como links internos já existentes no site e a edição/idioma do Yoast instalada. O motor entrega automaticamente os principais sinais on-page controláveis pelo artigo.

Como gerar o pacote instalável:
- O código-fonte em `blog-privilege-ai/` é a fonte oficial versionada.
- Na raiz do repositório, execute `./scripts/build-plugin.sh`.
- O arquivo instalável será criado em `dist/blog-privilege-ai.zip`.
- A pasta `dist/` não é versionada, evitando arquivos binários incompatíveis com a criação do pull request.
- Para validar o contrato de integração SEO, execute `./tests/test-seo-contract.sh`.
