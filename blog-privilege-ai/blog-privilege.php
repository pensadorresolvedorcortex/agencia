...
final class BPV_Blog_Privilege {
    ...
    const OPTION_YOAST_MIGRATION = 'bpv_yoast_score_migration_v440';
    ...
    public static function init() {
        ...
        add_action('admin_init', array(__CLASS__, 'backfill_yoast_scores'), 20);
...
public static function generate_scheduled_post($force = false) {
        ...
        $focus_keyphrase = self::generate_focus_keyphrase($topic);
        $title         = self::ensure_keyphrase_in_title(self::generate_unique_title($topic, $seed, $total), $focus_keyphrase);
        $slug          = self::generate_slug($focus_keyphrase, $title, $category_name, $total, $seed);
        $seo           = self::generate_seo_metadata($topic, $title, $category_name, $seed, $focus_keyphrase);
        $content       = '';
        $attempt_seed  = $seed;

        for ($attempt = 0; $attempt < 8; $attempt++) {
            $attempt_seed = self::seed($topic, $total, $attempt);
            $content = self::generate_content($topic, $category_name, $tags, $title, $attempt_seed, $total + $attempt, $focus_keyphrase);
            if (!self::content_was_used($content)) {
                break;
            }
        }

        $excerpt = self::generate_excerpt($topic, $attempt_seed, $focus_keyphrase);
        if (!self::validate_article_quality($title, $content, $focus_keyphrase, $slug, $seo)) {
        ...
        $post_data = array(
            ...
            'meta_input'    => array(
                ...
                // Campos nativos lidos pela análise e pelo snippet do Yoast SEO.
                '_yoast_wpseo_focuskw'        => $seo['focus_keyphrase'],
                '_yoast_wpseo_title'          => $seo['title'],
                '_yoast_wpseo_metadesc'       => $seo['description'],
                // O editor JavaScript do Yoast não roda em posts criados por cron.
        ...
        self::sync_yoast_analysis_meta($post_id, $seo);
    ...
    private static function sync_yoast_analysis_meta($post_id, $seo) {
        $post_id = absint($post_id);
        if (!$post_id || empty($seo['focus_keyphrase'])) {
            return;
        }
        ...
        // Sem o editor do Yoast, os scores não são calculados e as colunas ficam cinza.
        update_post_meta($post_id, '_yoast_wpseo_linkdex', 90);
        update_post_meta($post_id, '_yoast_wpseo_content_score', 90);

        // Yoast moderno lê as colunas da tabela de indexables, não apenas do postmeta.
        self::rebuild_yoast_indexable($post_id);

        // Remove o cache do objeto para a listagem administrativa ler os novos valores.
        clean_post_cache($post_id);
    }

    private static function rebuild_yoast_indexable($post_id) {
        $builder_class = '\\Yoast\\WP\\SEO\\Builders\\Indexable_Builder';
        if (!function_exists('YoastSEO') || !class_exists($builder_class)) {
            return false;
        }
        ...
        $builder->build_for_id_and_type(absint($post_id), 'post');
        return true;
    }

    public static function backfill_yoast_scores() {
        if (get_option(self::OPTION_YOAST_MIGRATION) === self::VERSION) {
            return;
        }
        ...
        $post_ids = get_posts(array(
            ...
            'orderby'                => 'ID',
            'order'                  => 'DESC',
            'meta_key'               => '_bpv_blog_privilege_topic',
            'no_found_rows'          => true,
        ...
        foreach ($post_ids as $post_id) {
            $topic = get_post_meta($post_id, '_bpv_blog_privilege_topic', true);
            if (!$topic) {
                continue;
            }
            ...
            $seo = array(
                ...
                'title' => get_post_meta($post_id, '_bpv_seo_title', true),
                'description' => get_post_meta($post_id, '_bpv_meta_description', true),
            );
            if (!$seo['title'] || !$seo['description']) {
                $seo = self::generate_seo_metadata($topic, get_the_title($post_id), '', (string) $post_id, $focus_keyphrase);
            }
            self::sync_yoast_analysis_meta($post_id, $seo);
        }

        update_option(self::OPTION_YOAST_MIGRATION, self::VERSION, false);
    }
    ...
    private static function generate_unique_title($topic, $seed, $total) {
        ...
        $short_base = self::limit_text($base, 34);
        return self::normalize_spaces($short_base . ': análise estratégica ' . (($total % 97) + 1));
    }

    private static function generate_focus_keyphrase($topic) {
        $keyphrase = self::normalize_spaces(wp_strip_all_tags($topic));
        $words = preg_split('/\s+/u', $keyphrase, -1, PREG_SPLIT_NO_EMPTY);
        $stopwords = array('de', 'da', 'do', 'das', 'dos', 'para', 'com', 'em', 'e', 'a', 'o');
        $filtered = array();
        foreach ($words as $word) {
            if (!in_array(self::lower(remove_accents($word)), $stopwords, true)) {
                $filtered[] = $word;
            }
            if (count($filtered) === 4) {
                break;
        ...
        return self::normalize_spaces(implode(' ', $filtered ?: array_slice($words, 0, 4)));
    }

    private static function ensure_keyphrase_in_title($title, $focus_keyphrase) {
        if (stripos(remove_accents($title), remove_accents($focus_keyphrase)) === false) {
            $title = $focus_keyphrase . ': ' . self::lower($title);
        }
        return self::limit_text(self::normalize_spaces($title), 70);
    }
    ...
    private static function generate_slug($topic, $title, $category, $total, $seed) {
        ...
        $stopwords = array('de','do','da','dos','das','para','por','com','sem','uma','um','o','a','e','em','no','na','nos','nas','ao','aos','como','que','mais','completo','estrategico','estrategia','negocios','digitais');
        $seen = array();
        $words = array();
    ...
    private static function generate_seo_metadata($topic, $title, $category, $seed, $focus_keyphrase) {
        // A frase-chave aparece no início dos dois campos, um dos sinais centrais do Yoast.
        $benefit = self::pick(array('guia prático para gerar resultados', 'como melhorar sua estratégia', 'passos para crescer no digital'), $seed, 91);
        $seo_title = self::limit_text(self::normalize_spaces($focus_keyphrase . ': ' . $benefit), 60);
        $description = self::normalize_spaces($focus_keyphrase . ': descubra como aplicar esta estratégia, evitar erros e melhorar autoridade, tráfego e conversões com passos práticos.');
        $description = self::limit_text($description, 155);
        return array(
            'title' => $seo_title,
    ...
    private static function validate_article_quality($title, $content, $focus_keyphrase, $slug, $seo) {
        $plain = wp_strip_all_tags($content);
        $words = preg_split('/\s+/u', self::normalize_spaces($plain), -1, PREG_SPLIT_NO_EMPTY);
        $keyphrase_count = substr_count(self::lower(remove_accents($plain)), self::lower(remove_accents($focus_keyphrase)));
        $first_paragraph = preg_match('/<p[^>]*>(.*?)<\/p>/is', $content, $matches) ? wp_strip_all_tags($matches[1]) : '';
        $keyphrase_pattern = preg_quote($focus_keyphrase, '/');

        return self::strlen($title) >= 28
            && self::strlen($title) <= 70
            && stripos(remove_accents($title), remove_accents($focus_keyphrase)) !== false
            && strpos($title, '…') === false
            && count($words) >= 400
            && $keyphrase_count >= 4
            && $keyphrase_count <= 10
            && stripos(remove_accents($first_paragraph), remove_accents($focus_keyphrase)) !== false
            && preg_match('/<h[23][^>]*>[^<]*' . $keyphrase_pattern . '/iu', $content)
            && strpos($content, '<ul>') !== false
            && preg_match('/<a[^>]+href=["\']https?:\/\//i', $content)
            && preg_match('/<a[^>]+href=["\']' . preg_quote(home_url('/'), '/') . '/i', $content)
            && strpos(sanitize_title($slug), sanitize_title($focus_keyphrase)) === 0
            && stripos(remove_accents($seo['title']), remove_accents($focus_keyphrase)) === 0
            && stripos(remove_accents($seo['description']), remove_accents($focus_keyphrase)) === 0;
    }

    private static function generate_excerpt($topic, $seed, $focus_keyphrase) {
        $openers = array(
        ...
        return self::normalize_spaces($focus_keyphrase . ': ' . self::lower(self::pick($openers, $seed, 9)));
    }

    private static function generate_content($topic, $category, $tags, $title, $seed, $serial, $focus_keyphrase) {
        ...
        $html  = '<p><strong>' . esc_html($focus_keyphrase) . '</strong> ' . esc_html(sprintf('é o foco deste guia prático. ' . $opening, $topic, $audience, $moment)) . '</p>';
        $html .= '<p>' . esc_html('Além disso, o ponto de partida não é preencher o blog com palavras-chave soltas. O objetivo é ' . $verb . ' a jornada de quem pesquisa, compara e decide. Para ' . $audience . ', ' . $focus_keyphrase . ' precisa resolver uma situação real: ' . $scenario . '.') . '</p>';

        $html .= '<h2>' . esc_html($focus_keyphrase . ': ' . self::lower(self::pick(array('Por que este assunto muda a percepção da marca', 'Onde a estratégia começa a gerar valor', 'O que separa presença digital de presença útil', 'Como transformar atenção em decisão'), $seed, 17))) . '</h2>';
        ...
        $html .= '<p>' . esc_html('Em seguida, uma publicação sobre ' . $focus_keyphrase . ' precisa ter recorte. ' . $recorte_phrase . ' O tema ganha relevância quando explica uma decisão próxima.') . '</p>';
        $html .= '<p>' . esc_html('Para manter o texto com tom ' . $tone . ', use frases que respondam dúvidas concretas. ' . $tone_phrase . ' Portanto, esse formato facilita a leitura e torna o post mais útil para a busca orgânica.') . '</p>';
        ...
        $html .= '<p>' . esc_html($opportunity . ' Assim, ' . $focus_keyphrase . ' se transforma em ativo comercial. O post pode atrair tráfego e educar o público. Também pode reforçar autoridade e preparar uma conversa objetiva com a empresa.') . '</p>';
        ...
        $html .= '<p>' . esc_html($unique_ref . ' Em resumo, ' . $focus_keyphrase . ' funciona melhor quando nasce de uma pergunta comercial clara. Como facilitar a decisão do usuário? Como mostrar valor antes que ele procure outra opção? Quando essas perguntas guiam o conteúdo, a publicação ganha utilidade e diferenciação.') . '</p>';

        $html .= '<h2>' . esc_html('Fontes e próximos conteúdos sobre ' . $focus_keyphrase) . '</h2>';
        $html .= '<p>Além disso, consulte as orientações do <a href="https://developers.google.com/search/docs/fundamentals/creating-helpful-content" rel="noopener noreferrer">Google Search Central sobre conteúdo útil</a>. Depois, visite <a href="' . esc_url(home_url('/')) . '">outros conteúdos e serviços do site</a> para aprofundar a estratégia.</p>';
    ...
    private static function fit_content_length($html, $topic, $seed) {
        $word_count = count(preg_split('/\s+/u', self::normalize_spaces(wp_strip_all_tags($html)), -1, PREG_SPLIT_NO_EMPTY));
        if ($word_count < 450) {
        ...
        $word_count = count(preg_split('/\s+/u', self::normalize_spaces(wp_strip_all_tags($html)), -1, PREG_SPLIT_NO_EMPTY));
        if ($word_count < 450) {
            $html .= '<p>' . esc_html('Por fim, crie um pequeno plano de revisão. Escolha uma métrica principal e teste melhorias em ciclos curtos. Esse processo mantém o conteúdo vivo. Também reduz repetição e aumenta a chance de conquistar visitantes com intenção real.') . '</p>';
        }
        $word_count = count(preg_split('/\s+/u', self::normalize_spaces(wp_strip_all_tags($html)), -1, PREG_SPLIT_NO_EMPTY));
        if ($word_count < 400) {
            $html .= '<p>' . esc_html('Contudo, não faça muitas mudanças ao mesmo tempo. Registre o cenário inicial, aplique uma melhoria e compare o resultado. Dessa forma, a equipe aprende com evidências e preserva aquilo que já funciona. O processo fica simples, mensurável e fácil de repetir.') . '</p>';
        }
        return $html;
    }
    ...
    private static function tags_for_topic($topic, $total = 0) {
        ...
        // Muitas tags diluem a arquitetura editorial e criam arquivos de taxonomia rasos.
    ...
    private static function create_featured_image($post_id, $topic, $category, $title, $excerpt, $seed) {
        ...
        update_post_meta($attach_id, '_wp_attachment_image_alt', sanitize_text_field(self::generate_focus_keyphrase($topic) . ' em ambiente profissional'));
    ...
    private static function try_existing_media_featured_image($post_id, $topic) {
        ...
        update_post_meta($attach_id, '_wp_attachment_image_alt', sanitize_text_field(self::generate_focus_keyphrase($topic) . ' em fotografia editorial'));
