...
final class BPV_Blog_Privilege {
    ...
    const LOCK_CLEANUP_HOOK     = 'bpv_blog_privilege_cleanup_generation_lock';
    const NUMBERED_PURGE_HOOK   = 'bpv_blog_privilege_trash_numbered_posts';
    const OPTION_ENABLED        = 'bpv_blog_privilege_enabled';
    const OPTION_INDEX          = 'bpv_blog_privilege_topic_index';
    const OPTION_TOPICS         = 'bpv_blog_privilege_topics';
    ...
    const OPTION_SLUG_HASHES    = 'bpv_blog_privilege_slug_hashes';
    ...
    const OPTION_PURGE_STATUS   = 'bpv_blog_privilege_numbered_purge_status';
    ...
    public static function init() {
        ...
        add_action(self::LOCK_CLEANUP_HOOK, array(__CLASS__, 'cleanup_generation_lock'));
        add_action(self::NUMBERED_PURGE_HOOK, array(__CLASS__, 'trash_numbered_posts_batch'));
    ...
    public static function activate() {
        ...
        if (get_option(self::OPTION_TOPICS, null) === null) {
            add_option(self::OPTION_TOPICS, self::default_topics(), '', false);
        }
        ...
        if (get_option(self::OPTION_SLUG_HASHES, null) === null) {
            add_option(self::OPTION_SLUG_HASHES, array(), '', false);
        }
    ...
    public static function deactivate() {
        ...
        wp_clear_scheduled_hook(self::LOCK_CLEANUP_HOOK);
        wp_clear_scheduled_hook(self::NUMBERED_PURGE_HOOK);
    ...
    public static function add_cron_interval($schedules) {
        ...
        $schedules['bpv_fifteen_minutes'] = array(
            'interval' => 900,
            'display'  => 'A cada 15 minutos - limpeza de trava Blog Privilége',
        );
        return $schedules;
    }
    ...
    private static function schedule_event() {
        ...
        if (!wp_next_scheduled(self::LOCK_CLEANUP_HOOK)) {
            wp_schedule_event(time() + 900, 'bpv_fifteen_minutes', self::LOCK_CLEANUP_HOOK);
        }
    }
    ...
    public static function handle_admin_actions() {
        ...
        if ($action === 'save_topics') {
            $topics = self::sanitize_topics(wp_unslash($_POST['bpv_topics'] ?? ''));
            if (empty($topics)) {
                add_settings_error('bpv_blog_privilege', 'bpv_topics_empty', 'Informe pelo menos um assunto para a geração dos posts.', 'error');
            } else {
                update_option(self::OPTION_TOPICS, $topics, false);
                $index = absint(get_option(self::OPTION_INDEX, 0));
                if ($index >= count($topics)) {
                    update_option(self::OPTION_INDEX, 0);
                }
                add_settings_error('bpv_blog_privilege', 'bpv_topics_saved', 'Assuntos do blog salvos com sucesso.', 'updated');
            }
        }

        if ($action === 'reset_topics') {
            update_option(self::OPTION_TOPICS, self::default_topics(), false);
            update_option(self::OPTION_INDEX, 0);
            add_settings_error('bpv_blog_privilege', 'bpv_topics_reset', 'Assuntos padrão restaurados.', 'updated');
        }
        ...
        if ($action === 'repair_slugs') {
            $result = self::repair_legacy_slugs();
            if (is_wp_error($result)) {
                add_settings_error('bpv_blog_privilege', 'bpv_slug_repair_error', $result->get_error_message(), 'error');
            } else {
                add_settings_error(
                    'bpv_blog_privilege',
                    'bpv_slug_repair_done',
                    sprintf(
                        'Auditoria concluída: %d slug(s) corrigido(s), %d já adequado(s) e %d conflito(s) enviado(s) para revisão manual. Os URLs antigos continuam com redirecionamento 301 nativo do WordPress.',
                        $result['repaired'],
                        $result['clean'],
                        $result['conflicts']
                    ),
                    $result['conflicts'] ? 'warning' : 'updated'
                );
            }
        }

        if ($action === 'trash_numbered_posts') {
            $pending = self::count_numbered_generated_posts();
            if ($pending < 1) {
                add_settings_error('bpv_blog_privilege', 'bpv_purge_empty', 'Nenhuma postagem do plugin com número no final da URL foi encontrada.', 'updated');
            } else {
                update_option(self::OPTION_PURGE_STATUS, array(
                    'status' => 'running',
                    'total' => $pending,
                    'processed' => 0,
                    'failed' => 0,
                ...
                self::trash_numbered_posts_batch();
                add_settings_error('bpv_blog_privilege', 'bpv_purge_started', 'Exclusão iniciada em lotes seguros. As postagens estão sendo enviadas para a Lixeira e podem ser restauradas pelo WordPress.', 'updated');
            }
        }
    }


    private static function last_featured_image_url($last_post) {
        if ($last_post) {
            $url = get_the_post_thumbnail_url($last_post, 'large');
            if ($url) {
                return $url;
            }
        }
        ...
        if (!empty($attachments[0])) {
            $url = wp_get_attachment_image_url(absint($attachments[0]), 'large');
            return $url ? $url : '';
        }
        return '';
    }

    private static function generated_post_rows() {
        global $wpdb;
        return $wpdb->get_results(
            $wpdb->prepare(
                "SELECT DISTINCT p.ID, p.post_title, p.post_name, p.post_content
                 FROM {$wpdb->posts} p
                 ...
                 ORDER BY p.ID ASC",
                '_bpv_blog_privilege_topic'
            ),
            ARRAY_A
        );
    }

    private static function audit_generated_posts() {
        global $wpdb;
        $where = $wpdb->prepare(
            " FROM {$wpdb->posts} p
              INNER JOIN {$wpdb->postmeta} pm ON pm.post_id = p.ID
        ...
        $total = absint($wpdb->get_var("SELECT COUNT(DISTINCT p.ID)" . $where));
        $duplicate_titles = absint($wpdb->get_var(
            "SELECT COUNT(*) FROM (SELECT p.post_title" . $where . " GROUP BY p.post_title HAVING COUNT(DISTINCT p.ID) > 1) bpv_titles"
        ));
        $duplicate_contents = absint($wpdb->get_var(
            "SELECT COUNT(*) FROM (SELECT MD5(p.post_content) content_hash" . $where . " GROUP BY content_hash HAVING COUNT(DISTINCT p.ID) > 1) bpv_contents"
        ));
        return array(
            'total' => $total,
            'numbered_slugs' => self::count_numbered_generated_posts(),
            'duplicate_titles' => $duplicate_titles,
            'duplicate_contents' => $duplicate_contents,
        );
    }

    private static function numbered_generated_post_ids($limit = 100) {
        global $wpdb;
    ...
    private static function count_numbered_generated_posts() {
        global $wpdb;
    ...
    public static function trash_numbered_posts_batch() {
        $status = get_option(self::OPTION_PURGE_STATUS, array());
        if (!is_array($status) || ($status['status'] ?? '') !== 'running') {
            return;
        }

        $ids = self::numbered_generated_post_ids(100);
        $trashed = 0;
        foreach ($ids as $post_id) {
            if (wp_trash_post($post_id)) {
                $trashed++;
                $status['processed'] = absint($status['processed'] ?? 0) + 1;
            } else {
                $status['failed'] = absint($status['failed'] ?? 0) + 1;
            }
        }

        $remaining = self::count_numbered_generated_posts();
        $status['remaining'] = $remaining;
        $status['updated_at'] = time();
        if ($remaining > 0 && $trashed > 0) {
            update_option(self::OPTION_PURGE_STATUS, $status, false);
            if (!wp_next_scheduled(self::NUMBERED_PURGE_HOOK)) {
                wp_schedule_single_event(time() + 10, self::NUMBERED_PURGE_HOOK);
            }
            return;
        }

        $status['status'] = $remaining > 0 ? 'paused' : 'complete';
        $status['finished_at'] = time();
        update_option(self::OPTION_PURGE_STATUS, $status, false);
    }

    private static function repair_legacy_slugs() {
        $rows = self::generated_post_rows();
        $result = array('repaired' => 0, 'clean' => 0, 'conflicts' => 0);

        foreach ($rows as $row) {
            $old_slug = (string) $row['post_name'];
            if (!preg_match('/-\d+$/', $old_slug)) {
                $result['clean']++;
                self::remember_slug($old_slug);
                continue;
            }

            $post_id = absint($row['ID']);
            $topic = (string) get_post_meta($post_id, '_bpv_blog_privilege_topic', true);
            $categories = get_the_category($post_id);
            $category = !empty($categories[0]->name) ? $categories[0]->name : '';
            $seed = (string) get_post_meta($post_id, '_bpv_blog_privilege_seed', true);
            if ($seed === '') {
                $seed = self::seed($topic, $post_id, 0);
            }
            $new_slug = self::generate_slug($topic, $row['post_title'], $category, $post_id, $seed . '|legacy-repair');

            if ($new_slug === '' || $new_slug === $old_slug || self::slug_exists($new_slug)) {
                $result['conflicts']++;
                continue;
            }

            $updated = wp_update_post(array('ID' => $post_id, 'post_name' => $new_slug), true);
            $saved_slug = is_wp_error($updated) ? '' : (string) get_post_field('post_name', $post_id);
            if (is_wp_error($updated) || $saved_slug !== $new_slug) {
                if ($saved_slug && $saved_slug !== $old_slug) {
                    wp_update_post(array('ID' => $post_id, 'post_name' => $old_slug));
                }
                $result['conflicts']++;
                continue;
            }

            // wp_update_post aciona o mecanismo _wp_old_slug do WordPress, que
            // preserva o endereço anterior com redirecionamento canônico 301.
            self::remember_slug($new_slug);
            $result['repaired']++;
        }
    ...
    public static function render_admin_page() {
        ...
        $next_topic = isset($topics[$index % $topic_count]) ? $topics[$index % $topic_count] : $topics[0];
        ...
        $last_image_url = self::last_featured_image_url($last_post);
        $current_lock = get_transient(self::LOCK_KEY);
        $lock_age = $current_lock ? max(0, time() - self::lock_started_at($current_lock)) : 0;
        $seo_audit = self::audit_generated_posts();
        $purge_status = get_option(self::OPTION_PURGE_STATUS, array());

        echo '<style>
        @import url("https://fonts.googleapis.com/css2?family=Inter:wght@500;600;700;800;900&display=swap");
        .bpv-saas{margin:18px 20px 0 0;min-height:760px;position:relative;overflow:hidden;border-radius:34px;padding:24px;background:radial-gradient(circle at 12% 8%,rgba(14,165,233,.20),transparent 30%),radial-gradient(circle at 88% 0%,rgba(124,58,237,.18),transparent 32%),linear-gradient(135deg,#f8fbff 0%,#eef4ff 46%,#ffffff 100%);box-shadow:0 30px 90px rgba(15,23,42,.16);color:#0f172a;font-family:Inter,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}.bpv-saas:before{content:"";position:absolute;inset:0;background:linear-gradient(120deg,rgba(255,255,255,.72),transparent 38%,rgba(255,255,255,.34));pointer-events:none}.bpv-shell{max-width:1180px;margin:0 auto;position:relative;z-index:1}.bpv-topbar{display:flex;align-items:center;justify-content:space-between;gap:16px;margin-bottom:18px}.bpv-brand{display:flex;align-items:center;gap:12px}.bpv-logo{width:44px;height:44px;border-radius:16px;display:grid;place-items:center;color:#fff;font-weight:900;letter-spacing:-.04em;background:linear-gradient(135deg,#0ea5e9,#7c3aed);box-shadow:0 16px 32px rgba(37,99,235,.28)}.bpv-brand strong{display:block;font-size:15px}.bpv-brand span,.bpv-pill{color:#64748b;font-size:12px}.bpv-pill{display:inline-flex;align-items:center;gap:8px;border-radius:999px;padding:9px 13px;background:rgba(255,255,255,.76);border:1px solid rgba(148,163,184,.24);box-shadow:0 10px 25px rgba(15,23,42,.07)}.bpv-hero,.bpv-card,.bpv-metric{background:rgba(255,255,255,.74);border:1px solid rgba(148,163,184,.26);box-shadow:0 18px 56px rgba(15,23,42,.10),inset 0 1px 0 rgba(255,255,255,.9);backdrop-filter:blur(18px);-webkit-backdrop-filter:blur(18px)}.bpv-hero{border-radius:30px;padding:28px;display:grid;grid-template-columns:minmax(0,1.2fr) minmax(280px,.8fr);gap:24px;margin-bottom:16px}.bpv-kicker{text-transform:uppercase;letter-spacing:.16em;color:#2563eb;font-size:12px;font-weight:900}.bpv-title{font-size:38px;line-height:1.02;margin:10px 0 12px;color:#0f172a;letter-spacing:-.05em}.bpv-sub{font-size:14px;line-height:1.65;max-width:780px;color:#475569;margin:0}.bpv-hero-actions{display:flex;flex-wrap:wrap;gap:10px;margin-top:18px}.bpv-status-dot{width:9px;height:9px;border-radius:999px;background:#22c55e;box-shadow:0 0 0 6px rgba(34,197,94,.12)}.bpv-preview{min-height:230px;border-radius:28px;background:linear-gradient(135deg,rgba(226,232,240,.72),rgba(255,255,255,.92));border:1px solid rgba(148,163,184,.30);position:relative;overflow:hidden;box-shadow:0 20px 50px rgba(15,23,42,.12)}.bpv-preview:before{content:"";position:absolute;inset:0;background:radial-gradient(circle at 20% 18%,rgba(14,165,233,.24),transparent 26%),linear-gradient(135deg,transparent,rgba(124,58,237,.12));z-index:1;pointer-events:none}.bpv-preview img{width:100%;height:100%;min-height:230px;object-fit:cover;display:block}.bpv-preview span{position:absolute;left:16px;bottom:14px;z-index:2;background:rgba(15,23,42,.78);color:#fff;border-radius:999px;padding:8px 13px;font-size:11px;font-weight:800;letter-spacing:.08em;text-transform:uppercase}.bpv-preview-empty{min-height:230px;display:flex;align-items:center;justify-content:center;color:#64748b;font-size:13px;text-align:center;padding:24px}.bpv-grid{display:grid;grid-template-columns:repeat(5,minmax(130px,1fr));gap:12px;margin:0 auto 16px}.bpv-metric{padding:16px;border-radius:22px;position:relative;overflow:hidden}.bpv-metric:after{content:"";position:absolute;right:-24px;top:-24px;width:80px;height:80px;border-radius:999px;background:rgba(37,99,235,.10)}.bpv-metric span{display:block;color:#64748b;font-size:11px;text-transform:uppercase;letter-spacing:.08em;font-weight:800}.bpv-metric strong{display:block;margin-top:6px;font-size:18px;color:#0f172a}.bpv-section-grid{display:grid;grid-template-columns:minmax(0,1fr) minmax(320px,.85fr);gap:14px;align-items:start}.bpv-card{padding:18px;border-radius:24px;margin-top:14px}.bpv-card h2{font-size:17px;color:#0f172a;margin:0 0 10px;letter-spacing:-.02em}.bpv-card p{color:#475569;line-height:1.55}.bpv-table{width:100%;border-collapse:separate;border-spacing:0 8px}.bpv-table th{color:#2563eb;text-align:left;width:185px;font-size:12px;text-transform:uppercase;letter-spacing:.06em}.bpv-table td{color:#334155;font-weight:650}.bpv-badge{display:inline-flex;align-items:center;border-radius:999px;padding:6px 11px;background:rgba(34,197,94,.13);color:#15803d;border:1px solid rgba(34,197,94,.22);font-weight:900}.bpv-actions{display:flex;gap:10px;flex-wrap:wrap;margin-top:12px}.bpv-saas .button{border-radius:999px!important;border:1px solid rgba(37,99,235,.18)!important;background:rgba(255,255,255,.88)!important;color:#1e293b!important;box-shadow:0 10px 25px rgba(15,23,42,.08)!important;font-weight:800!important}.bpv-saas .button-primary{background:linear-gradient(135deg,#0ea5e9,#7c3aed)!important;color:#fff!important;border:none!important}.bpv-form-table th{color:#1d4ed8;font-size:12px;text-transform:uppercase;letter-spacing:.06em}.bpv-form-table textarea,.bpv-form-table input{width:100%;background:rgba(248,250,252,.92)!important;color:#0f172a!important;border:1px solid rgba(148,163,184,.32)!important;border-radius:18px!important;box-shadow:inset 0 1px 0 rgba(255,255,255,.75)!important;padding:12px!important;font-weight:650}.bpv-topic-card{background:linear-gradient(135deg,rgba(255,255,255,.86),rgba(239,246,255,.76))}.bpv-art-card{background:linear-gradient(135deg,rgba(15,23,42,.94),rgba(30,41,59,.9));border-color:rgba(125,211,252,.24);color:#fff}.bpv-art-card h2,.bpv-art-card .bpv-form-table th{color:#fff}.bpv-art-card p{color:#cbd5e1}.bpv-art-card textarea,.bpv-art-card input{background:rgba(15,23,42,.76)!important;color:#fff!important;border-color:rgba(255,255,255,.20)!important}.bpv-description{color:#64748b!important;font-size:12px!important;margin-top:8px!important}.bpv-footer-note{border-left:4px solid #0ea5e9;background:rgba(14,165,233,.08)}
        @media(max-width:1100px){.bpv-hero,.bpv-section-grid{grid-template-columns:1fr}.bpv-grid{grid-template-columns:repeat(2,1fr)}}@media(max-width:640px){.bpv-saas{padding:14px;border-radius:22px}.bpv-topbar{align-items:flex-start;flex-direction:column}.bpv-hero{padding:18px}.bpv-title{font-size:28px}.bpv-grid{grid-template-columns:1fr}.bpv-table th,.bpv-table td{display:block;width:auto}.bpv-table th{margin-top:8px}}
        </style>';
        echo '<div class="wrap"><div class="bpv-saas"><div class="bpv-shell">';
        settings_errors('bpv_blog_privilege');
        echo '<div class="bpv-topbar"><div class="bpv-brand"><div class="bpv-logo">BP</div><div><strong>Blog Privilège AI</strong><span>Console SaaS premium para operação editorial</span></div></div><div class="bpv-pill"><span class="bpv-status-dot"></span>' . esc_html($enabled === 'yes' ? 'Automação ativa' : 'Automação pausada') . '</div></div>';
        echo '<section class="bpv-hero"><div><div class="bpv-kicker">Premium SaaS Dashboard • 2026</div><h1 class="bpv-title">Centro editorial com IA, SEO e governança de assuntos</h1><p class="bpv-sub">Uma experiência visual mais limpa, executiva e organizada para controlar geração automática, diagnóstico operacional, direção de arte e temas do blog em um único painel.</p><div class="bpv-hero-actions"><span class="bpv-pill">Próximo tema: ' . esc_html($next_topic) . '</span><span class="bpv-pill">' . esc_html($topic_count) . ' assuntos cadastrados</span></div></div><div class="bpv-preview">' . ($last_image_url ? '<img src="' . esc_url($last_image_url) . '" alt="Última imagem destacada gerada"><span>Última imagem</span>' : '<div class="bpv-preview-empty">A última imagem destacada aparecerá aqui após a próxima geração bem-sucedida.</div>') . '</div></section>';
        ...
        echo '</div>';
        echo '<div class="bpv-section-grid">';
        ...
        echo '<section class="bpv-card"><h2>Auditoria de URLs e indexabilidade</h2><p>Verificação local dos posts criados pelo Blog Privilège antes do acompanhamento no Google Search Console.</p><table class="bpv-table"><tbody>';
        echo '<tr><th>Posts auditados</th><td>' . esc_html($seo_audit['total']) . '</td></tr>';
        echo '<tr><th>Slugs com sufixo</th><td>' . esc_html($seo_audit['numbered_slugs']) . '</td></tr>';
        echo '<tr><th>Títulos repetidos</th><td>' . esc_html($seo_audit['duplicate_titles']) . '</td></tr>';
        echo '<tr><th>Conteúdos repetidos</th><td>' . esc_html($seo_audit['duplicate_contents']) . '</td></tr>';
        echo '<tr><th>Visibilidade</th><td>' . esc_html(get_option('blog_public', '1') === '1' ? 'Permitida aos mecanismos de busca' : 'Bloqueada nas configurações do WordPress') . '</td></tr>';
        echo '</tbody></table><form method="post" class="bpv-actions">';
        wp_nonce_field('bpv_blog_privilege_action', 'bpv_blog_privilege_nonce');
        echo '<button class="button button-primary" name="bpv_blog_privilege_action" value="repair_slugs">Corrigir slugs antigos com sufixo</button></form><p class="description bpv-description">A correção atua somente nos posts identificados pelo plugin. Títulos ou conteúdos repetidos são apenas sinalizados para revisão, nunca apagados automaticamente.</p></section>';
        echo '<section class="bpv-card"><h2>Excluir postagens com número na URL</h2><p>Envia para a Lixeira, em lotes de 100, somente as postagens geradas pelo Blog Privilège cujo slug termina em número. Posts de outros autores ou plugins não são afetados.</p>';
        if (is_array($purge_status) && !empty($purge_status['status'])) {
            echo '<p><strong>Status:</strong> ' . esc_html($purge_status['status']) . ' — ' . esc_html(absint($purge_status['processed'] ?? 0)) . ' excluída(s), ' . esc_html(absint($purge_status['remaining'] ?? $seo_audit['numbered_slugs'])) . ' restante(s), ' . esc_html(absint($purge_status['failed'] ?? 0)) . ' falha(s).</p>';
        }
        echo '<form method="post" class="bpv-actions" onsubmit="return confirm(\'Confirma o envio de todas as postagens do Blog Privilège com número no final da URL para a Lixeira? Esta ação pode afetar URLs já indexadas.\');">';
        wp_nonce_field('bpv_blog_privilege_action', 'bpv_blog_privilege_nonce');
        echo '<button class="button" style="color:#b91c1c!important;border-color:#fecaca!important" name="bpv_blog_privilege_action" value="trash_numbered_posts"' . ($seo_audit['numbered_slugs'] < 1 ? ' disabled' : '') . '>Excluir ' . esc_html($seo_audit['numbered_slugs']) . ' postagem(ns) com número na URL</button></form><p class="description bpv-description">A exclusão não é permanente: revise a Lixeira antes de esvaziá-la. O processamento continua pelo WP-Cron para evitar timeout com milhares de posts.</p></section>';
        echo '<div class="bpv-section-grid">';
        echo '<section class="bpv-card bpv-topic-card"><h2>Assuntos do Blog</h2>';
        echo '<p>Edite a lista de temas usados na geração automática. Use um assunto por linha; para excluir, remova a linha desejada. A ordem define a sequência das próximas postagens.</p>';
        echo '<form method="post">';
        wp_nonce_field('bpv_blog_privilege_action', 'bpv_blog_privilege_nonce');
        echo '<input type="hidden" name="bpv_blog_privilege_action" value="save_topics">';
        echo '<table class="form-table bpv-form-table">';
        echo '<tr><th>Assuntos ativos<br><small>' . esc_html($topic_count) . ' temas cadastrados</small></th><td><textarea class="large-text" rows="14" name="bpv_topics">' . esc_textarea(implode("\n", $topics)) . '</textarea><p class="description bpv-description">Exemplos atuais: sites, SEO, marketing digital, branding, design, e-commerce e naming.</p></td></tr>';
        echo '</table>';
        echo '<div class="bpv-actions"><button class="button button-primary">Salvar assuntos</button></form>';
        echo '<form method="post">';
        wp_nonce_field('bpv_blog_privilege_action', 'bpv_blog_privilege_nonce');
        echo '<button class="button" name="bpv_blog_privilege_action" value="reset_topics">Restaurar assuntos padrão</button></form></div>';
        echo '</section>';
        echo '<section class="bpv-card bpv-footer-note"><h2>Regras aplicadas</h2><p>Posts automáticos com conteúdo estruturado, títulos profissionais orientados por benefício, dor e curiosidade legítima, SEO title separado, meta description, slug curto e fotografia editorial premium. O fluxo evita ilustrações, cartoons, vetores, avatares, texto, logos e watermarks.</p></section>';
...
public static function generate_scheduled_post($force = false) {
        ...
        $seo           = self::generate_seo_metadata($topic, $title, $category_name, $seed);
        $content       = '';
        $attempt_seed  = $seed;
        ...
        // Nunca publica o último candidato apenas porque as tentativas acabaram.
        if (!$content || self::content_was_used($content)) {
            update_option(self::OPTION_LAST_ERR, 'Publicação cancelada: não foi possível produzir conteúdo editorial inédito em 8 tentativas.');
            self::release_generation_lock($lock_token);
            return new WP_Error('bpv_duplicate_content', 'Conteúdo repetido detectado. Nenhum post foi publicado.');
        }
        $content_signature = md5(self::content_signature_text($content));

        if (self::title_exists($title) || self::slug_exists($slug)) {
            update_option(self::OPTION_LAST_ERR, 'Publicação cancelada pela barreira anti-duplicidade de título/slug.');
            self::release_generation_lock($lock_token);
            return new WP_Error('bpv_duplicate_identity', 'Título ou slug já existente. Nenhum post foi publicado.');
        }
        ...
        $post_data = array(
            ...
            'meta_input'    => array(
                ...
                '_bpv_content_signature'      => $content_signature,
            ),
        );
        ...
        // O WordPress acrescenta “-2” quando encontra colisão. Isso não é aceito:
        // a inserção é revertida, em vez de deixar uma URL que denuncia duplicidade.
        $saved_slug = (string) get_post_field('post_name', $post_id);
        if ($saved_slug !== $slug) {
            wp_delete_post($post_id, true);
            update_option(self::OPTION_LAST_ERR, 'Publicação revertida: o WordPress alterou o slug planejado por colisão.');
            self::release_generation_lock($lock_token);
            return new WP_Error('bpv_slug_collision', 'Colisão de slug detectada. Nenhum post foi publicado.');
        }
    ...
    public static function cleanup_generation_lock() {
        $lock = get_transient(self::LOCK_KEY);
        if ($lock && self::lock_is_stale($lock)) {
            delete_transient(self::LOCK_KEY);
            update_option(self::OPTION_LAST_ERR, 'Trava órfã liberada automaticamente pela rotina de 15 minutos.');
        }
    }
    ...
    private static function topics() {
        ...
        return self::sanitize_topics($topics);
    }
    ...
    private static function sanitize_topics($topics) {
        ...
        foreach ($topics as $topic) {
            $topic = self::normalize_spaces(sanitize_text_field($topic));
            if ($topic !== '' && !in_array($topic, $clean, true)) {
                $clean[] = $topic;
            }
        }
    ...
    private static function compact_topic($topic) {
        ...
        $exact = array(
            'Planejamento estratégico de sites' => 'Planejamento de sites',
            'Arquitetura e estrutura de páginas' => 'Arquitetura de páginas',
            'Experiência do usuário (UX)' => 'Experiência do usuário',
            'Interface e design de sites (UI)' => 'Design de interfaces',
            'Sites profissionais para pequenas empresas' => 'Sites para pequenas empresas',
            'Responsividade para dispositivos móveis' => 'Sites responsivos',
            'Google Meu Negócio / Perfil da Empresa' => 'Perfil da Empresa no Google',
            'Estratégias para aparecer antes dos concorrentes' => 'Superar concorrentes no Google',
            'Estratégias digitais para empresas' => 'Estratégia digital empresarial',
            'Conversão de clientes pela internet' => 'Conversão de clientes online',
            'Escalabilidade através do digital' => 'Escala no ambiente digital',
            'Tipografia e percepção de marca' => 'Tipografia de marca',
            'Confiança através da marca' => 'Confiança na marca',
            'Gestão da identidade visual' => 'Gestão da identidade visual',
            'Atualização de marcas antigas' => 'Atualização de marcas',
            'Recuperação da percepção da marca' => 'Recuperação da marca',
            'Erros de marcas desatualizadas' => 'Erros de marcas desatualizadas',
            'Design profissional versus amador' => 'Design profissional',
            'Fotografias e apresentação de produtos' => 'Fotos de produtos',
            'Integração de sistemas de venda' => 'Integração de vendas',
            'Diferenciação através do nome' => 'Diferenciação pelo nome',
            'Posicionamento através da linguagem' => 'Linguagem de marca',
            'Registro e proteção de nomes' => 'Proteção de nomes de marca',
        );
        ...
        $replacements = array(
            ...
            'Experiência do usuário' => 'UX',
            'Interface e design' => 'Design de interface',
            'dispositivos móveis' => 'celulares',
            'Estratégias digitais' => 'Estratégia digital',
            'Identidade visual corporativa' => 'Identidade visual',
            'Posicionamento através' => 'Posicionamento por',
            'Diferenciação através' => 'Diferenciação por',
        );

        $compact = str_ireplace(array_keys($replacements), array_values($replacements), $topic);
        $compact = self::normalize_spaces($compact);

        // Títulos profissionais não devem carregar reticências no tema principal.
        return self::limit_text($compact, 42);
    }

    private static function title_topic_phrase($topic) {
        $phrase = self::lower(self::compact_topic($topic));
        $acronyms = array(
            '/\bseo\b/u' => 'SEO',
            '/\bux\b/u' => 'UX',
            '/\bui\b/u' => 'UI',
            '/\bcrm\b/u' => 'CRM',
            '/\bcta\b/u' => 'CTA',
            '/\bwordpress\b/u' => 'WordPress',
            '/\bwoocommerce\b/u' => 'WooCommerce',
            '/\bgoogle\b/u' => 'Google',
            '/\binstagram\b/u' => 'Instagram',
            '/\bfacebook\b/u' => 'Facebook',
        );
        return preg_replace(array_keys($acronyms), array_values($acronyms), $phrase);
    }

    private static function title_strategy($topic) {
        $t = self::lower(remove_accents($topic));
        $strategy = array(
            'verbs' => array(
                'gerar mais oportunidades comerciais',
                'aumentar conversões com mais clareza',
                'fortalecer a confiança do cliente',
                'transformar atenção em resultado',
            ),
            'risks' => array(
                'afastam clientes prontos para decidir',
                'reduzem a percepção de valor',
                'enfraquecem a conversão',
                'fazem a empresa perder oportunidades',
            ),
            'problems' => array(
                'atenção sem resultado',
                'presença digital sem direção',
                'visitas sem conversão',
            ...
            'good' => array(
                'uma estratégia que gera resultado',
                'uma presença que inspira confiança',
                'uma marca que conquista atenção',
            ),
            'bad' => array(
                'uma execução que passa despercebida',
                'uma presença digital genérica',
                'uma estratégia sem impacto',
            ),
        );

        if (strpos($t, 'seo') !== false || strpos($t, 'google') !== false || strpos($t, 'busca') !== false || strpos($t, 'palavras-chave') !== false || strpos($t, 'search console') !== false || strpos($t, 'trafego organico') !== false) {
            $strategy['verbs'] = array('atrair tráfego realmente qualificado', 'conquistar posições melhores no Google', 'transformar buscas em oportunidades', 'aumentar cliques orgânicos');
            $strategy['risks'] = array('derrubam o alcance orgânico', 'impedem o Google de entender a página', 'fazem bons conteúdos perderem posições', 'reduzem cliques qualificados');
            $strategy['problems'] = array('impressões sem cliques', 'conteúdo sem posição', 'tráfego sem intenção', 'páginas invisíveis');
            $strategy['outcomes'] = array('autoridade orgânica', 'tráfego qualificado', 'mais oportunidades', 'crescimento previsível');
            $strategy['good'] = array('páginas que conquistam posições', 'conteúdos que respondem melhor', 'uma estratégia que gera tráfego');
            $strategy['bad'] = array('páginas que o Google ignora', 'conteúdos que não recebem cliques', 'uma estratégia baseada em achismo');
        } elseif (strpos($t, 'e-commerce') !== false || strpos($t, 'loja') !== false || strpos($t, 'checkout') !== false || strpos($t, 'carrinho') !== false || strpos($t, 'produto') !== false || strpos($t, 'compra') !== false) {
            $strategy['verbs'] = array('converter mais visitas em pedidos', 'reduzir o abandono de compra', 'aumentar vendas sem depender de desconto', 'fortalecer a confiança na compra');
            $strategy['risks'] = array('aumentam o abandono de carrinho', 'fazem clientes desistirem da compra', 'reduzem o valor percebido dos produtos', 'derrubam a conversão da loja');
            $strategy['problems'] = array('visitas sem pedidos', 'interesse sem compra', 'carrinhos abandonados', 'produtos sem valor percebido');
            $strategy['outcomes'] = array('mais pedidos', 'uma compra mais segura', 'conversões maiores', 'clientes mais confiantes');
            $strategy['good'] = array('uma loja que facilita a compra', 'uma experiência que converte', 'uma vitrine que gera confiança');
            $strategy['bad'] = array('uma loja que cria dúvidas', 'uma experiência que perde pedidos', 'uma vitrine que parece genérica');
        } elseif (strpos($t, 'marca') !== false || strpos($t, 'branding') !== false || strpos($t, 'identidade') !== false || strpos($t, 'rebranding') !== false || strpos($t, 'naming') !== false || strpos($t, 'nome') !== false || strpos($t, 'tipografia') !== false || strpos($t, 'cores') !== false) {
            $strategy['verbs'] = array('aumentar o valor percebido da marca', 'ser lembrado pelos clientes certos', 'construir uma presença mais desejada', 'transformar identidade em vantagem competitiva');
            $strategy['risks'] = array('fazem a marca parecer genérica', 'reduzem o valor percebido', 'dificultam a lembrança da empresa', 'enfraquecem o posicionamento');
            $strategy['problems'] = array('uma marca esquecível', 'identidade sem personalidade', 'comunicação sem consistência', 'percepção de baixo valor');
            $strategy['outcomes'] = array('uma marca memorável', 'maior valor percebido', 'posicionamento mais forte', 'diferenciação real');
            $strategy['good'] = array('uma marca que ocupa espaço na mente', 'uma identidade que transmite valor', 'um posicionamento difícil de copiar');
            $strategy['bad'] = array('uma marca que parece igual às outras', 'uma identidade sem direção', 'um visual que não sustenta valor');
        } elseif (strpos($t, 'site') !== false || strpos($t, 'landing') !== false || strpos($t, 'ux') !== false || strpos($t, 'ui') !== false || strpos($t, 'design') !== false || strpos($t, 'responsiv') !== false || strpos($t, 'pagina') !== false || strpos($t, 'performance') !== false) {
            $strategy['verbs'] = array('converter mais visitantes em clientes', 'reduzir atritos na jornada do usuário', 'transmitir confiança nos primeiros segundos', 'transformar navegação em oportunidade');
            $strategy['risks'] = array('derrubam conversões', 'afastam clientes nos primeiros segundos', 'fazem o site parecer amador', 'aumentam a rejeição');
            $strategy['problems'] = array('visitas sem contato', 'um site bonito que não vende', 'navegação com atrito', 'atenção sem ação');
            $strategy['outcomes'] = array('mais conversões', 'uma experiência convincente', 'mais contatos qualificados', 'confiança imediata');
            $strategy['good'] = array('um site que conduz à decisão', 'uma experiência que gera confiança', 'uma página que converte');
            $strategy['bad'] = array('um site que apenas ocupa espaço', 'uma experiência que cria dúvidas', 'uma página que perde oportunidades');
        } elseif (strpos($t, 'marketing') !== false || strpos($t, 'lead') !== false || strpos($t, 'funil') !== false || strpos($t, 'clientes') !== false || strpos($t, 'aquisicao') !== false || strpos($t, 'conteudo') !== false || strpos($t, 'digital') !== false) {
            $strategy['verbs'] = array('atrair clientes com maior intenção', 'transformar audiência em oportunidades', 'reduzir desperdício na aquisição', 'criar crescimento mais previsível');
            $strategy['risks'] = array('atraem público sem intenção de compra', 'aumentam o custo de aquisição', 'geram movimento sem resultado', 'desperdiçam boas oportunidades');
            $strategy['problems'] = array('audiência sem oportunidade', 'campanhas sem direção', 'conteúdo sem impacto comercial', 'crescimento imprevisível');
            $strategy['outcomes'] = array('leads mais qualificados', 'aquisição mais eficiente', 'crescimento previsível', 'mais oportunidades comerciais');
            $strategy['good'] = array('uma estratégia que cria demanda', 'um funil que conduz à decisão', 'conteúdo que prepara a venda');
            $strategy['bad'] = array('uma estratégia que só gera alcance', 'um funil cheio de vazamentos', 'conteúdo que ninguém lembra');
        }

        return $strategy;
    }

    private static function title_candidate_score($title, $topic, $seed) {
        $length = self::strlen($title);
        $normalized = self::lower(remove_accents($title));
        $score = 0;

        if ($length >= 45 && $length <= 68) {
            $score += 12;
        } elseif ($length >= 38 && $length <= 74) {
            $score += 7;
        } else {
            $score -= 8;
        }

        if (preg_match('/\b[3579]\b/u', $title)) {
            $score += 4;
        }
        ...
        foreach (array('como ', 'erros', 'sinais', 'decisões', 'antes de', 'o que separa', 'resultado', 'clientes', 'convers', 'crescimento', 'confiança', 'oportunidades') as $hook) {
            if (strpos($normalized, remove_accents($hook)) !== false) {
                $score += 2;
            }
        }

        foreach (array('segredo', 'chocante', 'ninguém te conta', 'imperdível', 'milagre', 'garantido', 'explodir vendas', 'viralizar fácil') as $clickbait) {
            if (strpos($normalized, remove_accents($clickbait)) !== false) {
                $score -= 30;
            }
        }

        // Pequena variação determinística impede que um único molde domine todos os posts.
        $score += abs(crc32($seed . '|' . $title)) % 5;
        return $score;
    }

    private static function generate_unique_title($topic, $seed, $total) {
        $base = self::compact_topic($topic);
        $phrase = self::title_topic_phrase($topic);
        $strategy = self::title_strategy($topic);
        $number = self::pick(array(3, 5, 7, 9), $seed, 70);
        $verb = self::pick($strategy['verbs'], $seed, 71);
        $risk = self::pick($strategy['risks'], $seed, 72);
        $problem = self::pick($strategy['problems'], $seed, 73);
        $outcome = self::pick($strategy['outcomes'], $seed, 74);
        $good = self::pick($strategy['good'], $seed, 75);
        $bad = self::pick($strategy['bad'], $seed, 76);

        $candidates = array(
            $base . ': como ' . $verb,
            'Como usar ' . $phrase . ' para ' . $verb,
            $number . ' ajustes em ' . $phrase . ' para ' . $verb,
            $number . ' erros em ' . $phrase . ' que ' . $risk,
            $base . ': ' . $number . ' decisões que impactam ' . $outcome,
            $base . ': como transformar ' . $problem . ' em ' . $outcome,
            $base . ': o que separa ' . $good . ' de ' . $bad,
            'Antes de investir em ' . $phrase . ', revise estes ' . $number . ' pontos',
            $base . ': ' . $number . ' sinais de que a estratégia precisa evoluir',
            'O que revisar em ' . $phrase . ' antes de investir mais',
            $base . ' sem improviso: o caminho para ' . $verb,
            $base . ': o método para ' . $verb,
            $base . ': o que empresas competitivas fazem diferente',
            $number . ' escolhas em ' . $phrase . ' para alcançar ' . $outcome,
            $base . ': por que boas estratégias ainda falham na execução',
            $base . ': o que realmente gera resultado',
            $base . ': estratégias para ganhar vantagem competitiva',
            $base . ': como evoluir sem desperdiçar investimento',
            $number . ' prioridades em ' . $phrase . ' para empresas que querem crescer',
        );
        ...
        foreach (array_unique($candidates) as $candidate) {
            ...
            $length = self::strlen($candidate);
            // Não corta títulos no meio de uma promessa: candidatos longos são descartados.
            if ($length < 28 || $length > 74 || strpos($candidate, '…') !== false) {
                continue;
            }

            $hash = md5(self::lower(remove_accents($candidate)));
            if (isset($used[$hash]) || self::title_exists($candidate)) {
                continue;
            }

            $ranked[] = array(
                'title' => $candidate,
                'score' => self::title_candidate_score($candidate, $topic, $seed),
            );
        }
        ...
        if (!empty($ranked[0]['title'])) {
            return $ranked[0]['title'];
        }

        // Fallback editorial limpo, sem números artificiais soltos no final do título.
        $fallbacks = array(
            $base . ': como gerar resultado com mais estratégia',
            $base . ': decisões práticas para empresas que querem crescer',
            'Como melhorar ' . $phrase . ' sem cair no conteúdo genérico',
        );
        foreach ($fallbacks as $fallback) {
            $fallback = self::normalize_spaces($fallback);
            if (self::strlen($fallback) > 74) {
                continue;
            }
            $hash = md5(self::lower(remove_accents($fallback)));
            if (!isset($used[$hash]) && !self::title_exists($fallback)) {
                return $fallback;
            }
        }

        // O identificador final é editorial e derivado da semente, não um contador
        // que produza títulos/URLs “-2”, “-3” ou semelhantes.
        $angles = array('diagnóstico avançado', 'plano de crescimento', 'guia de decisão', 'análise de oportunidades');
        foreach ($angles as $offset => $angle) {
            $fallback = self::normalize_spaces(self::limit_text($base, 34) . ': ' . self::pick($angles, $seed, 90 + $offset));
            if (!self::title_exists($fallback)) {
                return $fallback;
            }
        }
        return self::normalize_spaces(self::limit_text($base, 30) . ': diagnóstico e plano de evolução');
    }

    private static function generate_slug($topic, $title, $category, $total, $seed) {
        // SEO Engine 4.3: URL curta, sem sufixos numéricos e validada contra todo o banco.
        ...
        $base_words = array_slice($words, 0, 6);
        $modifiers = array('guia', 'diagnostico', 'metodo', 'planejamento', 'decisoes', 'crescimento', 'conversao', 'autoridade');
        $candidates = array(implode('-', $base_words));
        foreach ($modifiers as $offset => $modifier) {
            $candidate_words = array_slice($base_words, 0, 5);
            $candidate_words[] = self::pick($modifiers, $seed, 110 + $offset);
            $candidates[] = implode('-', array_unique($candidate_words));
        }
        foreach (array_unique($candidates) as $candidate) {
            $candidate = sanitize_title($candidate);
            while (strlen($candidate) > 60 && count($base_words) > 3) {
                array_pop($base_words);
                $candidate = sanitize_title(implode('-', $base_words));
            }
            if (count(array_filter(explode('-', $candidate))) >= 3 && !self::slug_exists($candidate)) {
                return $candidate;
            }
        }
        return '';
    ...
    private static function generate_seo_metadata($topic, $title, $category, $seed) {
        // O título SEO preserva a promessa editorial e evita complementos genéricos repetidos.
        $seo_title = self::limit_text(self::normalize_spaces($title), 60);
    ...
    private static function validate_article_quality($title, $content) {
        ...
        return self::strlen($title) >= 28 && self::strlen($title) <= 74 && strpos($title, '…') === false && substr_count($content, '<h2>') >= 4 && strpos($content, '<ul>') !== false && self::strlen($plain) >= 2500;
    }
    ...
    private static function create_featured_image($post_id, $topic, $category, $title, $excerpt, $seed) {
        ...
        if (is_wp_error($image_meta)) {
            ...
            if (!is_wp_error($existing_id)) {
                self::remember_image_log('existing-media-library');
                return $existing_id;
            }
        }
        if (is_wp_error($image_meta)) {
    ...
    private static function try_ai_contextual_photo($filepath, $topic, $category, $title, $excerpt, $seed) {
        if (get_transient(self::AI_IMAGE_BACKOFF_KEY)) {
            return new WP_Error('bpv_ai_backoff', 'IA de imagem em espera temporária por limite externo recente.');
        }
        ...
        if (is_wp_error($saved)) {
            $message = self::lower($saved->get_error_message());
            if (strpos($message, 'too many requests') !== false || strpos($message, '429') !== false || strpos($message, 'rate') !== false) {
                set_transient(self::AI_IMAGE_BACKOFF_KEY, time(), HOUR_IN_SECONDS);
            }
            return $saved;
        }
    ...
    private static function try_wikimedia_business_photo($filepath, $topic, $seed) {
        if (!function_exists('wp_remote_get')) {
            return new WP_Error('bpv_no_http', 'HTTP do WordPress indisponível.');
        }
        require_once ABSPATH . 'wp-admin/includes/file.php';
        ...
        $queries = array(
            'business team office laptop meeting photograph',
            'people working office laptop photograph',
            'marketing team meeting office photograph',
            'professional team workplace laptop photograph',
        );
        $offset = abs(crc32($topic . $seed . '|wikimedia')) % count($queries);
        $queries = array_merge(array_slice($queries, $offset), array_slice($queries, 0, $offset));
        ...
        foreach ($queries as $query) {
            ...
            if (is_wp_error($response) || absint(wp_remote_retrieve_response_code($response)) >= 300) {
                continue;
            }
            $data = json_decode(wp_remote_retrieve_body($response), true);
            ...
            foreach ($data['query']['pages'] as $page) {
                $info = !empty($page['imageinfo'][0]) ? $page['imageinfo'][0] : array();
                $candidate = !empty($info['thumburl']) ? $info['thumburl'] : (!empty($info['url']) ? $info['url'] : '');
                $mime = !empty($info['mime']) ? $info['mime'] : '';
                $width = !empty($info['thumbwidth']) ? absint($info['thumbwidth']) : (!empty($info['width']) ? absint($info['width']) : 0);
                $height = !empty($info['thumbheight']) ? absint($info['thumbheight']) : (!empty($info['height']) ? absint($info['height']) : 0);
                if (!$candidate || strpos($mime, 'image/') !== 0 || $width < 1200 || $height < 675) {
                    continue;
                }
        ...
        return new WP_Error('bpv_wikimedia_not_found', 'Não foi possível obter foto do Wikimedia Commons.');
    }



    private static function try_curated_unsplash_photo($filepath, $topic, $seed) {
        if (!function_exists('wp_remote_get')) {
            return new WP_Error('bpv_no_http', 'HTTP do WordPress indisponível.');
        }
        require_once ABSPATH . 'wp-admin/includes/file.php';
    ...
    private static function try_existing_media_featured_image($post_id, $topic) {
        ...
        $candidates = array();
        if ($site_icon && wp_attachment_is_image($site_icon)) {
            $candidates[] = $site_icon;
        }
        $attachments = get_posts(array(
            'post_type' => 'attachment',
            'post_mime_type' => 'image',
    ...
    private static function content_was_used($html) {
        ...
        // A assinatura em postmeta torna a memória anti-repetição permanente e
        // consultável mesmo que opções antigas tenham sido podadas.
        $post_id = $wpdb->get_var($wpdb->prepare(
            "SELECT post_id FROM {$wpdb->postmeta} WHERE meta_key = %s AND meta_value = %s LIMIT 1",
            '_bpv_content_signature',
            $hash
        ));
        if ($post_id) {
            return true;
        ...
        // Compatibilidade com artigos anteriores à assinatura permanente.
        $exact_post = $wpdb->get_var($wpdb->prepare(
            "SELECT ID FROM {$wpdb->posts} WHERE post_type = %s AND post_status NOT IN ('trash', 'auto-draft') AND post_content = %s LIMIT 1",
            'post',
            $html
        ));
        if ($exact_post) {
    ...
    private static function title_exists($title) {
        global $wpdb;
        $normalized = self::lower(remove_accents(self::normalize_spaces($title)));
        $used = get_option(self::OPTION_TITLE_HASHES, array());
        if (is_array($used) && isset($used[md5($normalized)])) {
            return true;
        }
        $titles = $wpdb->get_col($wpdb->prepare(
            "SELECT post_title FROM {$wpdb->posts} WHERE post_type = %s AND post_status NOT IN ('trash', 'auto-draft') AND post_title = %s LIMIT 1",
            'post',
            $title
        ));
        return !empty($titles);
    ...
    private static function slug_exists($slug) {
        ...
        $used = get_option(self::OPTION_SLUG_HASHES, array());
        if (is_array($used) && isset($used[md5($slug)])) {
            return true;
        }
        $post_id = $wpdb->get_var($wpdb->prepare(
    ...
    private static function remember_slug($slug) {
        $used = get_option(self::OPTION_SLUG_HASHES, array());
        if (!is_array($used)) {
            $used = array();
        }
        $used[md5(sanitize_title($slug))] = time();
        update_option(self::OPTION_SLUG_HASHES, self::trim_assoc($used, 2000), false);
    }
    ...
    private static function safe_substr($text, $start, $length = null) {
        ...
        if (is_array($characters)) {
            $slice = $length === null ? array_slice($characters, $start) : array_slice($characters, $start, $length);
