...
final class BPV_Blog_Privilege {
    ...
    public static function render_admin_page() {
        ...
        echo '<h2 style="margin-top:24px;">Diagnóstico da última geração</h2>';
        ...
        foreach (array('Artigo','SEO','Slug','Imagem','Publicação') as $label) {
            ...
            echo '<tr><th>' . esc_html($label) . '</th><td>' . esc_html($status) . ($message ? ' — ' . esc_html($message) : '') . '</td></tr>';
        }
        ...
        echo '<hr><h2>Diretor de Arte IA Premium</h2>';
        echo '<p>Configura a identidade visual usada pela geração automática das imagens.</p>';
...
public static function generate_scheduled_post($force = false) {
    ...
    private static function create_featured_image($post_id, $topic, $category, $title, $excerpt, $seed) {
        ...
        $image_meta = new WP_Error('bpv_image_not_started', 'Geração de imagem ainda não iniciada.');
    ...
    private static function validate_generated_image($filepath, $title, $topic) {
        ...
        return filesize($filepath) > 50000;
    }
