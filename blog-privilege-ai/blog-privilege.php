...
final class BPV_Blog_Privilege {
    ...
    private static function audit_generated_posts() {
        ...
        foreach ($rows as $row) {
            if (preg_match('/-\d+$/', (string) $row['post_name'])) {
                $numbered++;
            }
            $title_key = md5(self::lower(remove_accents(self::normalize_spaces($row['post_title']))));
            $content_key = md5(self::content_signature_text($row['post_content']));
            $titles[$title_key] = isset($titles[$title_key]) ? $titles[$title_key] + 1 : 1;
            $contents[$content_key] = isset($contents[$content_key]) ? $contents[$content_key] + 1 : 1;
        }

        return array(
            'total' => count($rows),
            'numbered_slugs' => $numbered,
            'duplicate_titles' => count(array_filter($titles, function($count) { return $count > 1; })),
            'duplicate_contents' => count(array_filter($contents, function($count) { return $count > 1; })),
        );
    }
