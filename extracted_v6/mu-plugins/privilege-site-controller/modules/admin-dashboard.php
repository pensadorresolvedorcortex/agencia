...
function privilege_diag_render_page() {
    ...
    <div class="wrap">
        ...
        <div class="notice notice-error" style="padding:16px;margin-top:16px;">
            <h2 style="margin-top:0;">Pendente de confirmação visual: CLS do hero no desktop (0,784)</h2>
            <p>
                O PSI de 23/08 (desktop) aponta a imagem de topo do Bold Builder
                (<code>bt_bb_section_top_section_coverage_image</code>) como responsável
                por 0,767 dos 0,784 de CLS — mesmo com <code>width</code>/<code>height</code>
                presentes no HTML. Isso é a assinatura de um "coverage image" cujo CSS
                estica/corta a imagem (<code>object-fit:cover</code>) para preencher uma
                seção com altura própria, independente da proporção real da imagem.
            </p>
            <p>
                A V5.2 aplicou um <code>aspect-ratio</code> calculado a partir do
                width/height da própria imagem — uma correção parcial e de baixo risco.
                Se o CSS do tema fixar a altura em vez de deixá-la automática, essa regra
                sozinha não resolve, e eu não tenho como saber qual dos dois casos é este
                sem ver a regra real. Ela aparece abaixo, extraída direto do
                <code>style.css</code> do tema/child theme instalado:
            </p>
            <?php if ( empty( $coverage_css ) ) : ?>
                <p style="margin-bottom:0;">
                    Nenhuma regra encontrada para essas classes no <code>style.css</code>
                    do tema ou do child theme — o estilo pode estar em outro arquivo (CSS
                    do Bold Builder/plugin, ou gerado dinamicamente). Se o problema
                    persistir depois de atualizar, o caminho mais direto é copiar aqui a
                    regra CSS de <code>.bt_bb_section_top_section_coverage_image</code> e
                    <code>.bt_bb_cell</code> direto do DevTools do navegador (aba
                    Elements/Computed, na Home).
                </p>
            <?php else : ?>
                <pre style="background:#1e1e1e;color:#d4d4d4;padding:14px;overflow-x:auto;max-height:320px;font-size:12px;line-height:1.5;"><?php
                foreach ( $coverage_css as $rule ) {
                    echo esc_html( $rule ) . "\n\n";
                }
                ?></pre>
                <p style="margin-bottom:0;color:#666;">
                    Extraído por busca de texto simples no seletor (não é um parser de
                    CSS completo) — pode incluir alguma regra de contexto mais amplo que
                    o necessário; o que importa são os blocos que mencionam
                    <code>coverage_image</code>, <code>bt_bb_cell</code> ou
                    <code>background_image_holder</code>.
                </p>
            <?php endif; ?>
        </div>

        <div class="notice notice-warning" style="padding:16px;margin-top:16px;">
            <h2 style="margin-top:0;">Ação manual de maior impacto: "Remove Unused CSS"</h2>
            <p>
                A auditoria de 21/08/2026 (PageSpeed Insights + HAR real) encontrou
                <strong>cerca de 158 KiB de CSS não usado</strong> na Home porque o
                "Remove Unused CSS" do FlyingPress está desligado. Nenhuma técnica de
                carregamento assíncrono deste pacote de mu-plugins reduz o que É
                baixado — só adia QUANDO é baixado. É o único ajuste capaz de atacar
                esse número diretamente, e sozinho é maior do que todos os outros
                achados somados.
            </p>
            <p>
            ...
            <p style="margin-bottom:0;">
                ...
                cache da Home</strong> e rode um novo PageSpeed Insights para confirmar
                a queda no CSS não usado.
            </p>
        </div>
