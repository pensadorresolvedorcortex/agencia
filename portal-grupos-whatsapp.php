...
final class Plugin {
    ...
    public static function categories(array $atts=[]): string { $atts=shortcode_atts(['limit'=>10,'count'=>0,'show_more'=>0],$atts,'pgw_categorias');$limit=max(1,min(100,(int)$atts['limit']));$show_more=!empty($atts['show_more']);$terms=get_terms(['taxonomy'=>self::CAT,'hide_empty'=>true,'number'=>$show_more?0:$limit,'orderby'=>'name']);if(is_wp_error($terms)||!$terms)return '<div class="pgw-empty">Nenhuma categoria disponível.</div>';$html='<nav class="pgw-categories" aria-label="Categorias" data-pgw-categories><ul>';foreach($terms as $index=>$term){$url=get_term_link($term);if(!is_wp_error($url))$html.='<li'.($show_more&&$index>=$limit?' hidden':'').'><a href="'.esc_url($url).'"'.self::category_color_style($term).'>'.esc_html($term->name).(!empty($atts['count'])?' <span>('.absint($term->count).')</span>':'').'</a></li>';}$html.='</ul>';if($show_more&&count($terms)>$limit)$html.='<button class="pgw-category-toggle" type="button" data-pgw-category-toggle aria-expanded="false"><span>Ver mais categorias</span><i aria-hidden="true"></i></button>';return $html.'</nav>'; }
