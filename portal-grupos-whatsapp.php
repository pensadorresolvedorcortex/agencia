...
final class Plugin {
    ...
    private static function send_otp(int $user_id, string $purpose, ?string $recipient = null, bool $preserve_account = false): bool {
        ...
        $subject = 'Seu código de segurança — Portal Grupos WhatsApp';
    ...
    private static function otp_email_html(string $code, string $purpose): string {
        $site = wp_specialchars_decode(get_bloginfo('name') ?: 'Portal Grupos WhatsApp', ENT_QUOTES);
        ...
        $logo_html = $logo ? '<img src="'.esc_url($logo).'" alt="'.esc_attr($site).'" style="max-width:168px;height:auto;display:block">' : '<strong style="font:900 20px Maven Pro,Arial,sans-serif;color:#fff;letter-spacing:-.03em">'.esc_html($site).'</strong>';
        return '<!doctype html><html><body style="margin:0;background:#07140d;padding:0;font-family:Maven Pro,Arial,sans-serif;color:#122017">'
            .'<span style="display:none!important;opacity:0;color:transparent;height:0;width:0;overflow:hidden">'.esc_html($preheader).'</span>'
            .'<table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="background:linear-gradient(135deg,#07140d 0%,#0d331f 52%,#05d491 100%);padding:34px 14px"><tr><td align="center">'
            .'<table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:620px;border-radius:30px;overflow:hidden;background:#ffffff;box-shadow:0 30px 90px rgba(0,0,0,.35)">'
            .'<tr><td style="padding:30px 34px;background:linear-gradient(135deg,#042214 0%,#12873d 62%,#00e0a4 100%)">'.$logo_html.'<p style="margin:18px 0 0;color:#d9fff0;font:800 12px Maven Pro,Arial,sans-serif;text-transform:uppercase;letter-spacing:.14em">Autenticação em 2 fatores</p><h1 style="margin:8px 0 0;color:#fff;font:900 32px/1 Maven Pro,Arial,sans-serif;letter-spacing:-.04em">'.esc_html($headline).'</h1></td></tr>'
            .'<tr><td style="padding:34px 34px 28px"><p style="margin:0 0 18px;color:#42524a;font:600 16px/1.55 Maven Pro,Arial,sans-serif">Digite o código abaixo na tela de confirmação. Depois da validação você será levado automaticamente para o grupo solicitado quando houver um destino pendente.</p>'
            ...
            .'<tr><td style="padding:22px 34px;background:#f4fbf7;color:#5c6962;font:700 12px/1.6 Maven Pro,Arial,sans-serif;text-align:center">© '.esc_html(gmdate('Y')).' '.esc_html($site).' · Portal independente, seguro e moderado.<br><a href="'.esc_url(home_url('/')).'" style="color:#16813b;text-decoration:none;font-weight:900">Acessar o portal</a></td></tr>'
