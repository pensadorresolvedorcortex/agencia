...
final class Plugin {
    ...
    private static function auth_flow_url(int $user_id, string $purpose, ?string $redirect = null): string {
        ...
        $redirect = wp_validate_redirect($redirect ?: (string)get_user_meta($user_id, 'pgw_pending_redirect', true), '');
        $args = ['pgw_flow' => $token];
    ...
    private static function consume_pending_redirect(int $user_id, ?string $fallback = null): string {
        $fallback = wp_validate_redirect((string)($fallback ?: ''), home_url('/minha-conta/'));
        $url=wp_validate_redirect((string)get_user_meta($user_id,'pgw_pending_redirect',true),$fallback);delete_user_meta($user_id,'pgw_pending_redirect');return $url;
    }

    private static function otp_redirect_from_request(): string {
        return wp_validate_redirect((string)wp_unslash($_REQUEST['pgw_redirect_to'] ?? ''), home_url('/minha-conta/'));
    }
