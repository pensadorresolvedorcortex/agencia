...
final class Plugin {
    ...
    private static function send_otp(int $user_id, string $purpose, ?string $recipient = null): bool {
        ...
        return wp_mail($recipient, 'Seu código de segurança — Portal Grupos WhatsApp', "Seu código é: {$code}\n\nEle expira em 10 minutos. Se você não solicitou este código, ignore esta mensagem.");
    }
    ...
    private static function public_query_args(array $extra=[]): array { return array_merge(['post_type'=>self::CPT,'post_status'=>'publish','posts_per_page'=>30,'meta_query'=>[['key'=>'_pgw_status','value'=>'approved']], 'orderby'=>['date'=>'DESC','ID'=>'DESC']], $extra); }
    ...
    public static function frontend_runtime(): void {
        ...
        wp_add_inline_script('pgw-runtime','(function(){"use strict";document.addEventListener("click",function(e){var tab=e.target.closest("[data-pgw-auth-tab]");if(!tab)return;var modal=tab.closest(".pgw-auth-modal"),target=tab.dataset.pgwAuthTab;modal.querySelectorAll("[data-pgw-auth-tab]").forEach(function(item){item.setAttribute("aria-selected",String(item===tab));});modal.querySelectorAll("[data-pgw-auth-panel]").forEach(function(panel){panel.hidden=panel.dataset.pgwAuthPanel!==target;});var heading=modal.querySelector("[data-pgw-auth-panel=\""+target+"\"] h2");if(heading)heading.focus({preventScroll:true});});document.addEventListener("keydown",function(e){if(e.key!=="Escape")return;var close=document.querySelector(".pgw-auth-modal .pgw-auth-close");if(close)close.click();});})();');
    }
    ...
    public static function catalog(array $atts=[]): string {
        ...
        $query=new \WP_Query(self::catalog_query_args($filters,['posts_per_page'=>max(1,min(100,(int)$atts['limit']))]));
        if(!$query->have_posts()) return '<div class="pgw-wrap"><p class="pgw-empty">Nenhum grupo encontrado.</p></div>';
        $featured=[]; $normal=[]; while($query->have_posts()){ $query->the_post(); $id=get_the_ID(); if (self::is_featured_active($id)) $featured[]=$id; else $normal[]=$id; } wp_reset_postdata();
        $html='<section class="pgw-wrap"><form class="pgw-search pgw-filters" method="get"><input type="search" name="pgw_q" value="'.esc_attr($filters['search']).'" placeholder="Buscar grupos"><input name="pgw_category" value="'.esc_attr($filters['category']).'" placeholder="Categoria (slug)"><select name="pgw_order"><option value="featured">Destaques</option><option value="recent"'.selected($filters['order'],'recent',false).'>Recentes</option><option value="updated"'.selected($filters['order'],'updated',false).'>Atualizados</option><option value="accessed"'.selected($filters['order'],'accessed',false).'>Mais acessados</option><option value="az"'.selected($filters['order'],'az',false).'>A–Z</option></select><button class="pgw-button">Filtrar</button></form>'; $priorities=[]; foreach($featured as $featured_id) $priorities[$featured_id]=absint(get_post_meta($featured_id,'_pgw_featured_priority',true)); $featured=(new Featured\FeaturedOrder())->order($priorities); $show=array_slice($featured,0,3); $used=[];
    ...
    public static function login_form(array $atts=[]): string {
        ...
        $message='';
        if($_SERVER['REQUEST_METHOD']==='POST'&&isset($_POST['pgw_login_nonce'])&&wp_verify_nonce(sanitize_text_field(wp_unslash((string)$_POST['pgw_login_nonce'])),'pgw_login')&&self::verify_form_proof('login',1)){
            $email=sanitize_email(wp_unslash((string)($_POST['pgw_email']??'')));$pass=(string)wp_unslash($_POST['pgw_password']??'');$identity=hash_hmac('sha256',strtolower($email?:'invalid'),wp_salt('nonce'));
            $allowed=self::rate_limit('login_ip','ip_'.self::request_ip_hash(),15,15*MINUTE_IN_SECONDS)&&self::rate_limit('login_email',$identity,8,15*MINUTE_IN_SECONDS);
            $user=$allowed&&$email?get_user_by('email',$email):false;
            if($user&&wp_check_password($pass,$user->user_pass,$user->ID)&&(string)get_user_meta($user->ID,'pgw_account_status',true)!=='suspended'){
                update_user_meta($user->ID,'pgw_pending_login',1);update_user_meta($user->ID,'pgw_pending_remember',!empty($_POST['pgw_remember'])?1:0);
                if(self::send_otp((int)$user->ID,'login')){self::audit('login_otp_requested','user',(int)$user->ID);self::frontend_redirect(self::auth_flow_url((int)$user->ID,'login'));}
                delete_user_meta($user->ID,'pgw_pending_login');delete_user_meta($user->ID,'pgw_pending_remember');
            }
            $message='<p role="alert">Não foi possível entrar com os dados informados.</p>';
        }
        return self::auth_modal('login',$message,'');
    }
    ...
    public static function ajax_groups(): void { check_ajax_referer('pgw_load_groups','nonce');
        ...
        $window=(new Search\PaginationWindow())->fromInput($_POST['offset']??0,$_POST['amount']??15);$input=wp_unslash($_POST);$input['pgw_q']=$input['search']??'';$q=new \WP_Query(self::catalog_query_args($input,['posts_per_page'=>$window['amount'],'offset'=>$window['offset']]));$html='';while($q->have_posts()){$q->the_post();$html.=self::card(get_the_ID(),false);}$returned=(int)$q->post_count;$has_more=(new Search\PaginationWindow())->hasMore($window['offset'],$returned,(int)$q->found_posts);wp_reset_postdata();wp_send_json_success(['html'=>$html,'has_more'=>$has_more,'next_offset'=>$window['offset']+$returned]); }
