...
require_once __DIR__ . '/src/Auth/PostOtpRedirectPolicy.php';
...
final class Plugin {
    ...
    public static function boot(): void {
        ...
        add_action('after_setup_theme', [self::class, 'theme_support'], 11);
        ...
        add_action('admin_post_pgw_frontend_auth', [self::class, 'process_auth_submission']);
        add_action('admin_post_nopriv_pgw_frontend_auth', [self::class, 'process_auth_submission']);
        ...
        add_action('wp_ajax_pgw_auth_form_proof', [self::class, 'ajax_auth_form_proof']);
        add_action('wp_ajax_nopriv_pgw_auth_form_proof', [self::class, 'ajax_auth_form_proof']);
        ...
        add_action('added_post_meta', [self::class, 'sync_thumbnail_meta'], 10, 4);
        add_action('updated_post_meta', [self::class, 'sync_thumbnail_meta'], 10, 4);
    ...
    public static function maybe_upgrade(): void {
        if ((int) get_option(self::OPTION_DB, 0) >= self::DB_VERSION) return;
        self::roles();
        self::create_tables();
    ...
    public static function theme_support(): void { add_theme_support('post-thumbnails', [self::CPT]); }
    ...
    private static function attach_group_image(string $field, int $group_id): int {
        ...
        update_post_meta($group_id, '_thumbnail_id', $attachment_id);
    ...
    private static function create_otp(int $user_id, string $purpose = 'login'): ?string {
        ...
        $inserted=$wpdb->insert($table, ['selector'=>$selector,'token_hash'=>hash_hmac('sha256', $selector, wp_salt('auth')),'code_hash'=>wp_hash_password($code),'user_id'=>$user_id,'purpose'=>$purpose,'email_hash'=>hash_hmac('sha256',(string) get_userdata($user_id)->user_email,wp_salt('auth')),'attempts'=>0,'max_attempts'=>5,'expires_at'=>gmdate('Y-m-d H:i:s', time() + 10 * MINUTE_IN_SECONDS),'created_at'=>current_time('mysql', true),'ip_hash'=>self::request_ip_hash(),'user_agent_hash'=>hash_hmac('sha256',(string) ($_SERVER['HTTP_USER_AGENT'] ?? ''),wp_salt('nonce'))], ['%s','%s','%s','%d','%s','%s','%d','%d','%s','%s','%s','%s']);
    ...
    private static function auth_flow_url(int $user_id, string $purpose, ?string $redirect = null): string {
        $token = (new Auth\FlowToken(wp_salt('auth')))->issue($user_id, $purpose, 15 * MINUTE_IN_SECONDS);
        $confirmation=get_page_by_path('confirmar-codigo',OBJECT,'page');$login=get_page_by_path('entrar',OBJECT,'page');$base=$confirmation?get_permalink($confirmation):($login?get_permalink($login):home_url('/entrar/'));
        $redirect = self::safe_post_auth_redirect($redirect ?: (string)get_user_meta($user_id, 'pgw_pending_redirect', true), '');
        $args = ['pgw_flow' => $token];
    ...
    private static function auth_page_slugs(): array { return ['entrar', 'criar-conta', 'confirmar-codigo']; }

    private static function post_otp_redirect_policy(): Auth\PostOtpRedirectPolicy {
        return new Auth\PostOtpRedirectPolicy(self::auth_page_slugs());
    }

    private static function is_auth_destination(string $url): bool {
        return self::post_otp_redirect_policy()->isAuthDestination($url);
    }

    private static function safe_post_auth_redirect(string $url, string $fallback): string {
        $policy = self::post_otp_redirect_policy();
        $fallback = $fallback !== '' ? wp_validate_redirect($fallback, home_url('/minha-conta/')) : '';
        if (!$policy->allows($fallback)) $fallback = home_url('/minha-conta/');
        $validated = wp_validate_redirect($url, $fallback);
        if (!$policy->allows($validated)) return $fallback !== '' ? $fallback : home_url('/minha-conta/');
        return $validated;
    }

    private static function requested_redirect(): string {
        $return = self::safe_post_auth_redirect((string)wp_unslash($_POST['pgw_auth_return']??''), home_url('/minha-conta/'));
        return self::safe_post_auth_redirect((string)wp_unslash($_POST['pgw_redirect_to']??''), $return);
    }

    private static function consume_pending_redirect(int $user_id, ?string $fallback = null): string {
        $fallback = self::safe_post_auth_redirect((string)($fallback ?: ''), home_url('/minha-conta/'));
        $url=self::safe_post_auth_redirect((string)get_user_meta($user_id,'pgw_pending_redirect',true),$fallback);delete_user_meta($user_id,'pgw_pending_redirect');return $url;
    }

    private static function otp_redirect_from_request(): string {
        return self::safe_post_auth_redirect((string)wp_unslash($_REQUEST['pgw_redirect_to'] ?? ''), home_url('/minha-conta/'));
    }
    ...
    private static function ensure_auth_runtime(): bool {
        if (!get_role('pgw_pending_member') || !get_role('pgw_member')) self::roles();
        global $wpdb;
        $required = [$wpdb->prefix.'pgw_auth_challenges', $wpdb->prefix.'pgw_rate_limits'];
        foreach ($required as $table) {
            $exists = (string)$wpdb->get_var($wpdb->prepare('SHOW TABLES LIKE %s', $wpdb->esc_like($table))) === $table;
            if (!$exists) { self::create_tables(); break; }
        }
        foreach ($required as $table) if ((string)$wpdb->get_var($wpdb->prepare('SHOW TABLES LIKE %s', $wpdb->esc_like($table))) !== $table) return false;
        return get_role('pgw_pending_member') instanceof \WP_Role && get_role('pgw_member') instanceof \WP_Role;
    }


    public static function process_otp_submission(): void {
        if ($_SERVER['REQUEST_METHOD'] !== 'POST' || empty($_POST['pgw_verify_nonce']) || empty($_POST['pgw_flow'])) return;
        echo self::verify_code();
        exit;
    }

    public static function process_auth_submission(): void {
        if($_SERVER['REQUEST_METHOD']!=='POST')return;$fallback=wp_validate_redirect((string)wp_unslash($_POST['pgw_auth_return']??wp_get_referer()),home_url('/entrar/'));if(!self::ensure_auth_runtime())self::frontend_redirect(add_query_arg('pgw_auth_error','configuration',$fallback));
    ...
    private static function send_otp(int $user_id, string $purpose, ?string $recipient = null, bool $preserve_account = false): bool {
        ...
        $body = self::otp_email_html($code, $purpose);
        add_filter('wp_mail_content_type', [self::class, 'html_mail_content_type']);
        $sent = wp_mail($recipient, $subject, $body);
        remove_filter('wp_mail_content_type', [self::class, 'html_mail_content_type']);
    ...
    public static function html_mail_content_type(): string { return 'text/html; charset=UTF-8'; }

    private static function otp_email_html(string $code, string $purpose): string {
        $headline = $purpose === 'registration' ? 'Finalize sua conta com segurança' : 'Confirme seu acesso';
        $preheader = 'Use o código de 6 dígitos para liberar sua sessão e acessar o grupo solicitado.';
        $logo = 'https://www.portalgruposwhatsapp.com.br/wp-content/uploads/2026/03/logo-dark.png';
        $logo_html = '<img src="'.esc_url($logo).'" alt="Grupos WhatsApp" style="width:220px;max-width:220px;height:auto;display:block;margin:0 auto">';
        return '<!doctype html><html><body style="margin:0;background:#eaeaea;padding:0;font-family:Maven Pro,Arial,sans-serif!important;color:#122017">'
            .'<span style="display:none!important;opacity:0;color:transparent;height:0;width:0;overflow:hidden">'.esc_html($preheader).'</span>'
            .'<table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="background:#eaeaea;padding:34px 14px;font-family:Maven Pro,Arial,sans-serif!important"><tr><td align="center">'
            .'<table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:620px;border-radius:30px;overflow:hidden;background:#ffffff;box-shadow:0 30px 90px rgba(0,0,0,.18)">'
            .'<tr><td style="padding:30px 34px;background:#ffffff;text-align:center">'.$logo_html.'</td></tr>'
            .'<tr><td style="padding:34px 34px 28px"><h1 style="margin:0 0 18px;color:#102d19;font:900 30px/1 Maven Pro,Arial,sans-serif;letter-spacing:-.04em;text-align:center">'.esc_html($headline).'</h1><p style="margin:0 0 18px;color:#42524a;font:600 16px/1.55 Maven Pro,Arial,sans-serif">Digite o código abaixo na tela de confirmação. Depois da validação você será levado automaticamente para o grupo solicitado quando houver um destino pendente.</p>'
            .'<div style="margin:24px 0;padding:22px;border:1px solid #d7f7e7;border-radius:24px;background:linear-gradient(135deg,#f2fff8,#fff);text-align:center"><p style="margin:0 0 8px;color:#16813b;font:900 12px Maven Pro,Arial,sans-serif;text-transform:uppercase;letter-spacing:.16em">Seu código</p><div style="font:900 44px/1 Maven Pro,Arial,sans-serif;letter-spacing:.22em;color:#07140d">'.esc_html($code).'</div></div>'
            .'<p style="margin:0;color:#6a756f;font:600 14px/1.55 Maven Pro,Arial,sans-serif">Este código expira em <strong>10 minutos</strong>. Se você não solicitou este acesso, ignore este e-mail com segurança.</p></td></tr>'
            .'<tr><td style="padding:22px 34px;background:#f6f6f6;color:#5c6962;font:700 12px/1.6 Maven Pro,Arial,sans-serif;text-align:center">© '.esc_html(gmdate('Y')).' Grupos WhatsApp - Todos os Direitos Reservados<br><a href="'.esc_url(home_url('/')).'" style="color:#16813b;text-decoration:none;font-weight:900">Acessar o portal</a></td></tr>'
    ...
    public static function restrict_member_admin(): void {
        ...
        $script=basename((string)($_SERVER['SCRIPT_NAME']??''));$action=sanitize_key((string)($_REQUEST['action']??''));
        if($script==='admin-post.php'&&str_starts_with($action,'pgw_'))return;
    ...
    public static function assets(): void {
        wp_enqueue_style('pgw-maven-pro','https://fonts.googleapis.com/css2?family=Maven+Pro:wght@400;500;600;700;800;900&display=swap',[],null);
        wp_enqueue_style('pgw-style',plugins_url('assets/dist/frontend.css',__FILE__),['pgw-maven-pro'],self::VERSION);
    }
    ...
    public static function admin_assets(string $hook): void { if (strpos($hook,'pgw')===false) return; wp_enqueue_style('pgw-maven-pro','https://fonts.googleapis.com/css2?family=Maven+Pro:wght@400;500;600;700;800;900&display=swap',[],null); wp_enqueue_style('pgw-style',plugins_url('assets/dist/frontend.css',__FILE__),['pgw-maven-pro'],self::VERSION); }
    ...
    public static function frontend_runtime(): void {
        ...
        wp_add_inline_script('pgw-runtime','(function(){"use strict";function apply(form,data){["started","identifier","proof"].forEach(function(key){var input=form.querySelector("[name=_pgw_"+(key==="identifier"?"form_id":key)+"]");if(input)input.value=data[key];});var trap=form.querySelector(".pgw-honeypot input");if(trap)trap.name=data.honeypot;}function refresh(modal){if(!modal||modal.dataset.pgwProofLoading)return;modal.dataset.pgwProofLoading="1";fetch(PGW_DATA.ajaxUrl,{method:"POST",credentials:"same-origin",headers:{"Content-Type":"application/x-www-form-urlencoded; charset=UTF-8"},body:"action=pgw_auth_form_proof"}).then(function(response){return response.json();}).then(function(result){if(!result.success)return;["login","register"].forEach(function(purpose){var form=modal.querySelector("[data-pgw-auth-panel="+purpose+"]");if(form&&result.data[purpose])apply(form,result.data[purpose]);});}).finally(function(){delete modal.dataset.pgwProofLoading;});}document.addEventListener("DOMContentLoaded",function(){document.querySelectorAll(".pgw-auth-modal").forEach(refresh);});document.addEventListener("click",function(e){if(e.target.closest(".pgw-open-auth,.pgw-open-login,.pgw-open-register"))refresh(document.querySelector(".pgw-auth-modal"));});})();');
    }
    ...
    public static function sync_business_status(int $post_id, \WP_Post $post, bool $update): void {
        ...
        if ($thumbnail_id) { update_post_meta($post_id, '_pgw_original_image_id', $thumbnail_id); update_post_meta($post_id, '_pgw_square_image_id', $thumbnail_id); update_post_meta($post_id, '_pgw_hero_image_id', $thumbnail_id); }
        self::invalidate_catalog_cache();
    }

    public static function sync_thumbnail_meta(int $meta_id, int $post_id, string $meta_key, mixed $meta_value): void { if ($meta_key !== '_thumbnail_id' || get_post_type($post_id) !== self::CPT) return; $thumbnail_id=absint($meta_value); if (!$thumbnail_id) return; update_post_meta($post_id,'_pgw_original_image_id',$thumbnail_id); update_post_meta($post_id,'_pgw_square_image_id',$thumbnail_id); update_post_meta($post_id,'_pgw_hero_image_id',$thumbnail_id); }
    ...
    public static function account_header(): string {
        if(!is_user_logged_in()){
            return '<details class="pgw-account-menu pgw-account-menu--guest pgw-account-menu--glass"><summary><span class="pgw-account-menu__user-icon" aria-hidden="true"><svg viewBox="0 0 24 24" focusable="false"><path d="M12 12.2a4.6 4.6 0 1 0 0-9.2 4.6 4.6 0 0 0 0 9.2Zm0 2.1c-4.6 0-8.3 2.5-8.3 5.5 0 .7.5 1.2 1.2 1.2h14.2c.7 0 1.2-.5 1.2-1.2 0-3-3.7-5.5-8.3-5.5Z"/></svg></span><span class="pgw-account-menu__identity"><small>Acesso rápido</small><strong>Entrar/Cadastrar</strong></span><span class="pgw-account-menu__spark" aria-hidden="true"></span></summary><nav aria-label="Acesso da conta"><div class="pgw-account-menu__panel-head"><span class="pgw-account-menu__dots" aria-hidden="true"><i></i><i></i><i></i></span><strong>Portal Grupos</strong></div><a class="pgw-open-login is-primary" href="'.esc_url(home_url('/entrar/')).'"><span>Entrar</span><small>Acessar minha conta</small></a><a class="pgw-open-register" href="'.esc_url(home_url('/criar-conta/')).'"><span>Criar conta</span><small>Cadastre-se em instantes</small></a><a href="'.esc_url(home_url('/recuperar-senha/')).'"><span>Recuperar senha</span><small>Receba um link seguro</small></a></nav></details>';
        }
        $user=wp_get_current_user();$account=home_url('/minha-conta/');
        return '<details class="pgw-account-menu pgw-account-menu--glass"><summary>'.self::avatar_html($user->ID,44,'pgw-account-menu__avatar').'<span class="pgw-account-menu__identity"><small>Minha conta</small><strong>'.esc_html($user->display_name).'</strong></span><span class="pgw-account-menu__spark" aria-hidden="true"></span></summary><nav aria-label="Menu da minha conta"><div class="pgw-account-menu__panel-head"><span class="pgw-account-menu__dots" aria-hidden="true"><i></i><i></i><i></i></span><strong>Portal Grupos</strong></div><a class="is-primary" href="'.esc_url($account).'#pgw-profile"><span>Perfil e nome</span><small>Dados públicos</small></a><a href="'.esc_url($account).'#pgw-avatar"><span>Foto do perfil</span><small>Imagem e avatar</small></a><a href="'.esc_url(home_url('/meus-grupos/')).'"><span>Meus grupos</span><small>Acompanhar envios</small></a><a href="'.esc_url(home_url('/enviar-grupo/')).'"><span>Enviar grupo</span><small>Novo cadastro</small></a><a href="'.esc_url($account).'#pgw-password"><span>Senha e segurança</span><small>2FA e senha</small></a><a href="'.esc_url($account).'#pgw-email"><span>Alterar e-mail</span><small>Confirmação segura</small></a><a href="'.esc_url($account).'#pgw-sessions"><span>Sessões ativas</span><small>Dispositivos conectados</small></a><a class="pgw-account-menu__logout" href="'.esc_url(wp_logout_url(home_url('/'))).'"><span>Sair</span><small>Encerrar sessão</small></a></nav></details>';
    }
    ...
    public static function submit_form(): string { if(!is_user_logged_in())return '<div class="pgw-empty">Para enviar um grupo, <a href="'.esc_url(home_url('/entrar/')).'">entre</a> ou <a href="'.esc_url(home_url('/criar-conta/')).'">crie uma conta</a>.</div>'; $message=''; $submitted_group_id=0; $categories=get_terms(['taxonomy'=>self::CAT,'hide_empty'=>false]); $types=get_terms(['taxonomy'=>self::TYPE,'hide_empty'=>false]); if($_SERVER['REQUEST_METHOD']==='POST'&&isset($_POST['pgw_submit_nonce'])&&wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['pgw_submit_nonce'])),'pgw_submit')&&self::verify_form_proof('submit',3)){ if(!self::rate_limit('group_submit','user_'.get_current_user_id(),8,DAY_IN_SECONDS))$message='<p role="alert">Limite de envios atingido. Tente novamente mais tarde.</p>';else{$title=self::trim_text(sanitize_text_field(wp_unslash($_POST['pgw_title']??'')),100);$url=self::sanitize_invite(wp_unslash($_POST['pgw_url']??''));$desc=self::trim_text(sanitize_textarea_field(wp_unslash($_POST['pgw_description']??'')),1000);$cat=absint($_POST['pgw_category']??0);$type=absint($_POST['pgw_type']??0);$rules=self::trim_text(sanitize_textarea_field(wp_unslash($_POST['pgw_rules']??'')),1000);if(!$title||!$url||!$desc||!$cat||!$type||empty($_POST['pgw_terms']))$message='<p role="alert">Preencha os campos obrigatórios e confirme que possui autorização para divulgar o link.</p>';else{$dup=new \WP_Query(['post_type'=>self::CPT,'post_status'=>'any','posts_per_page'=>1,'meta_key'=>'_pgw_invite_url','meta_value'=>$url,'fields'=>'ids']);if($dup->have_posts())$message='<p role="alert">Este link já foi enviado.</p>';else{$id=wp_insert_post(['post_type'=>self::CPT,'post_status'=>'pending','post_title'=>$title,'post_content'=>$desc,'post_author'=>get_current_user_id()],true);if(!is_wp_error($id)){update_post_meta($id,'_pgw_invite_url',$url);update_post_meta($id,'_pgw_invite_hash',hash_hmac('sha256',$url,wp_salt('auth')));update_post_meta($id,'_pgw_owner_id',get_current_user_id());update_post_meta($id,'_pgw_status','pending');update_post_meta($id,'_pgw_rules',$rules);wp_set_object_terms($id,[$cat],self::CAT);wp_set_object_terms($id,[$type],self::TYPE);self::attach_group_image('pgw_group_image',$id);self::audit('group_submitted','group',$id,[],['status'=>'pending']);$submitted_group_id=(int)$id;$message='<p role="status">Seu grupo foi enviado para moderação.</p>';}}}}} if($submitted_group_id){$target=home_url('/meus-grupos/');return '<div class="pgw-submit-success-modal" role="dialog" aria-modal="true" aria-labelledby="pgw-submit-success-title"><section><span aria-hidden="true">✓</span><h2 id="pgw-submit-success-title">Grupo enviado para análise</h2><p>Seu grupo foi criado e já aparece em Meus Grupos com o status <strong>Em análise</strong>. Assim que for aprovado, você poderá gerir as informações e acompanhar o desempenho.</p><div class="pgw-submit-success-actions"><a class="pgw-button" href="'.esc_url($target).'">Ver meus grupos</a><button class="pgw-button pgw-button--secondary" type="button" data-pgw-success-close>Fechar</button></div></section><script>(function(){var modal=document.querySelector(".pgw-submit-success-modal"),target=modal?modal.querySelector(".pgw-button"):null,close=modal?modal.querySelector("[data-pgw-success-close]"):null;function go(){if(target)window.location.href=target.href;}if(close)close.addEventListener("click",go);setTimeout(go,9000);}());</script></div>';} $cat_options='<option value="">Selecione</option>';foreach($categories as $term)$cat_options.='<option value="'.absint($term->term_id).'">'.esc_html($term->name).'</option>'; $type_options='<option value="">Selecione</option>';foreach($types as $term)$type_options.='<option value="'.absint($term->term_id).'">'.esc_html($term->name).'</option>'; return '<div class="pgw-wrap pgw-submit-page"><form method="post" enctype="multipart/form-data" class="pgw-submit-form"><header class="pgw-submit-head"><span class="pgw-submit-head__seal" aria-hidden="true">✓</span><div class="pgw-submit-head__copy"><h2>Dados do grupo</h2><p>Todo envio passa por moderação antes de ser publicado.</p></div></header>'.$message.wp_nonce_field('pgw_submit','pgw_submit_nonce',true,false).self::form_proof_fields('submit').'<p class="pgw-submit-field pgw-submit-field--name"><label>Nome<input name="pgw_title" maxlength="100" autocomplete="organization" placeholder="Ex.: Achadinhos da Shopee" required></label></p><p class="pgw-submit-field pgw-submit-field--url"><label>Link HTTPS do WhatsApp<input type="url" name="pgw_url" inputmode="url" placeholder="https://chat.whatsapp.com/..." required></label></p><p class="pgw-submit-field pgw-submit-field--category"><label>Categoria<select name="pgw_category" required>'.$cat_options.'</select></label></p><p class="pgw-submit-field pgw-submit-field--type"><label>Tipo<select name="pgw_type" required>'.$type_options.'</select></label></p><p class="pgw-submit-field pgw-submit-field--image"><label>Imagem quadrada <small>JPEG, PNG ou WebP; até 5 MB</small><input type="file" name="pgw_group_image" accept="image/jpeg,image/png,image/webp"></label></p><p class="pgw-submit-field pgw-submit-field--description"><label>Descrição<textarea name="pgw_description" maxlength="1000" rows="5" placeholder="Explique o propósito do grupo, temas aceitos e para quem ele é indicado." required></textarea></label></p><p class="pgw-submit-field pgw-submit-field--rules"><label>Regras do grupo <textarea name="pgw_rules" maxlength="1000" rows="4" placeholder="Informe regras importantes para novos participantes."></textarea></label></p><p class="pgw-submit-field pgw-submit-field--terms"><label><input type="checkbox" name="pgw_terms" value="1" required> <span>Confirmo que sou proprietário ou possuo autorização para divulgar este grupo.</span></label></p><footer class="pgw-submit-footer"><small>Revise o link antes de enviar. Ele só ficará visível para usuários autenticados.</small><button class="pgw-button" type="submit">Enviar para análise</button></footer></form></div>'; }

    public static function login_form(array $atts=[]): string {
        if(!empty($_REQUEST['pgw_flow']))return self::verify_code();
        if(is_user_logged_in())return '<p>Você já está conectado. <a href="'.esc_url(home_url('/minha-conta/')).'">Acessar minha conta</a></p>';
        $error=sanitize_key((string)($_GET['pgw_auth_error']??''));$register_errors=['register','register_proof','register_data','register_rate','register_exists','register_create','otp'];$register_error=in_array($error,$register_errors,true);$register_messages=['register_proof'=>'Atualize a página e envie novamente. A proteção do formulário expirou.','register_data'=>'Confira nome, e-mail, senha e aceite os termos para continuar. O telefone é opcional.','register_rate'=>'Muitas tentativas de cadastro. Aguarde alguns minutos e tente novamente.','register_exists'=>'Este e-mail já possui uma conta. Use Entrar ou recupere a senha.','register_create'=>'Não foi possível criar a conta agora. Tente novamente em instantes.','otp'=>'Conta criada, mas não foi possível iniciar o código. Tente reenviar em instantes.'];$login_message=$error&&!$register_error?'<p role="alert">Não foi possível entrar. Confira os dados ou tente novamente em instantes.</p>':'';$register_message=$register_error?'<p role="alert">'.esc_html($register_messages[$error]??'Não foi possível concluir o cadastro. Confira os dados ou solicite um novo código.').'</p>':'';
    ...
    public static function register_form(): string {
        ...
        $error=sanitize_key((string)($_GET['pgw_auth_error']??''));$messages=['register_proof'=>'Atualize a página e envie novamente. A proteção do formulário expirou.','register_data'=>'Confira nome, e-mail, senha e aceite os termos para continuar. O telefone é opcional.','register_rate'=>'Muitas tentativas de cadastro. Aguarde alguns minutos e tente novamente.','register_exists'=>'Este e-mail já possui uma conta. Use Entrar ou recupere a senha.','register_create'=>'Não foi possível criar a conta agora. Tente novamente em instantes.','otp'=>'Conta criada, mas não foi possível iniciar o código. Tente reenviar em instantes.'];$message=$error?'<p role="alert">'.esc_html($messages[$error]??'Não foi possível concluir o cadastro. Confira os dados ou tente novamente em instantes.').'</p>':'';
    ...
    public static function verify_code(): string {
        ...
        $post_auth_redirect = self::otp_redirect_from_request();
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (!wp_verify_nonce($nonce, 'pgw_verify') || $posted_purpose !== $purpose || strlen($code) !== 6) {
                ...
                wp_set_auth_cookie($user_id, true, is_ssl()); wp_safe_redirect(self::consume_pending_redirect($user_id, $post_auth_redirect)); exit;
        ...
        $redirect_field = '<input type="hidden" name="pgw_redirect_to" value="'.esc_attr($post_auth_redirect).'">';
        $resend = '<button class="pgw-button pgw-button--secondary pgw-resend-otp" type="button" data-flow="'.esc_attr($flow_token).'">Reenviar código</button><span class="pgw-otp-status" aria-live="polite"></span>';
        return '<div class="pgw-wrap pgw-otp-wrap"><form method="post" class="pgw-auth pgw-auth--otp"><img class="pgw-otp-logo" src="https://www.portalgruposwhatsapp.com.br/wp-content/uploads/2026/03/logo-dark.png" alt="Grupos WhatsApp"><h2>Confirme seu código</h2><p class="pgw-auth-lead">Digite o código enviado por e-mail para liberar sua conta e continuar para o grupo.</p>'.$message.wp_nonce_field('pgw_verify','pgw_verify_nonce',true,false).'<input type="hidden" name="pgw_flow" value="'.esc_attr($flow_token).'">'.$redirect_field.'<label class="pgw-otp-code"><span>Código de seis dígitos</span><input inputmode="numeric" autocomplete="one-time-code" name="pgw_code" maxlength="6" pattern="[0-9]{6}" placeholder="000000" required></label><div class="pgw-otp-actions"><button class="pgw-button" type="submit">Confirmar e continuar</button>'.$resend.'</div></form></div>';
    }
    ...
    public static function ajax_auth_form_proof(): void {
        if(!self::ensure_auth_runtime()||!self::rate_limit('auth_proof_issue','ip_'.self::request_ip_hash(),60,HOUR_IN_SECONDS))wp_send_json_error(['message'=>'Não foi possível atualizar o formulário.'],429);
        $guard=new Security\FormProof(wp_salt('nonce'));$payload=[];
        foreach(['login','register'] as $purpose){$proof=$guard->issue($purpose);$payload[$purpose]=['started'=>$proof['started'],'identifier'=>$proof['identifier'],'proof'=>$proof['proof'],'honeypot'=>$proof['honeypot']];}
